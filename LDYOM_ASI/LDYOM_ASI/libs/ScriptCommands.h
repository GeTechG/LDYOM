...
namespace pluginmy {
	...
	public:
...
#endif
			strcpy(scriptHandle.m_szName, "plg-sdk");
			scriptHandle.m_bIsMission = false;
			scriptHandle.m_bUseMissionCleanup = false;
			scriptHandle.m_bNotFlag = (commandId >> 15) & 1;
			// our script code
...
#ifdef GTASA
			scriptHandle.m_pBaseIP = scriptHandle.m_pCurrentIP = code.GetData();
#else
			script.m_nIp = reinterpret_cast<int>(code.GetData()) - reinterpret_cast<int>(CRunningScript::GetScriptSpaceBase());
#endif
			scriptHandle.ProcessOneCommand();
			code.SaveResultVariables(&scriptHandle);
			return scriptHandle.m_bCondResult ? true : false;
		}
		...
		static bool CallCommandById(unsigned int commandId, ArgTypes... arguments) {
			...
			scriptHandle.m_pBaseIP = scriptHandle.m_pCurrentIP = code.GetData();
#else
			scriptHandle.m_nIp = reinterpret_cast<int>(code.GetData()) - reinterpret_cast<int>(CRunningScript::GetScriptSpaceBase());
#endif
			scriptHandle.ProcessOneCommand();
			code.SaveResultVariables(&scriptHandle);
			return scriptHandle.m_bCondResult ? true : false;
