...
---@class LDNodeEditorTextPrintStringNowNode : LDNodeEditorNode
---@field text string
---@field time integer

local TEXT_PRINT_STRING_NOW_NODE_TYPE = "core.text_print_string_now"

---@type LDNodeEditorNodeType
local textPrintStringNowNode = {
    typeName = TEXT_PRINT_STRING_NOW_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTextPrintStringNowNode
        local newNode = {
            id = newNodeId,
            nodeType = TEXT_PRINT_STRING_NOW_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTextPrintStringNowNode
    ...
    run = function(editor, context, node, inputValues)
		...
		TextOp.printStringNow(text, math.floor(time))
        return {1}
    end
}

return textPrintStringNowNode
