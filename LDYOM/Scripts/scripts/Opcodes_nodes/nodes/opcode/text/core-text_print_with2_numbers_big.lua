...
---@class LDNodeEditorTextPrintWith2NumbersBigNode : LDNodeEditorNode
---@field key string
---@field num1 integer
---@field num2 integer
...
local TEXT_PRINT_WITH2_NUMBERS_BIG_NODE_TYPE = "core.text_print_with2_numbers_big"

---@type LDNodeEditorNodeType
local textPrintWith2NumbersBigNode = {
    typeName = TEXT_PRINT_WITH2_NUMBERS_BIG_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTextPrintWith2NumbersBigNode
        local newNode = {
            id = newNodeId,
            nodeType = TEXT_PRINT_WITH2_NUMBERS_BIG_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[5], "duration", function ()
            node.duration = editor.dataTypes[node.inputs[5].type].drawEditValue(node.duration, "##durationEdit", fontScale * 100)
        end);

		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[6], "style", function ()
            local name = "<Not set>";
    ...
    run = function(editor, context, node, inputValues)
		...
		TextOp.printWith2NumbersBig(key, math.floor(num1), math.floor(num2), math.floor(duration), style)
        return {1}
    end
}

return textPrintWith2NumbersBigNode
