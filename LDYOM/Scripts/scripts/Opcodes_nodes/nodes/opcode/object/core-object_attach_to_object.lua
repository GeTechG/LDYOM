...
---@class LDNodeEditorObjectAttachToObjectNode : LDNodeEditorNode
---@field self_ integer
...
local OBJECT_ATTACH_TO_OBJECT_NODE_TYPE = "core.object_attach_to_object"

---@type LDNodeEditorNodeType
local objectAttachToObjectNode = {
    typeName = OBJECT_ATTACH_TO_OBJECT_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorObjectAttachToObjectNode
        local newNode = {
            id = newNodeId,
            nodeType = OBJECT_ATTACH_TO_OBJECT_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorObjectAttachToObjectNode
    ...
    run = function(editor, context, node, inputValues)
		...
		ObjectOp.attachToObject(self_, handle, xOffset, yOffset, zOffset, xRotation, yRotation, zRotation)
        return {1}
    end
}

return objectAttachToObjectNode
