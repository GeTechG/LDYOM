...
---@class LDNodeEditorObjectLocate2DNode : LDNodeEditorNode
---@field self_ integer
---@field x number
---@field y number
...
local OBJECT_LOCATE2_D_NODE_TYPE = "core.object_locate2_d"

---@type LDNodeEditorNodeType
local objectLocate2DNode = {
    typeName = OBJECT_LOCATE2_D_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorObjectLocate2DNode
        local newNode = {
            id = newNodeId,
            nodeType = OBJECT_LOCATE2_D_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorObjectLocate2DNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local result = ObjectOp.locate2D(self_, x, y, xRadius, yRadius, drawSphere)
        return {result}
    end
}

return objectLocate2DNode
