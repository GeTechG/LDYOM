...
---@class LDNodeEditorObjectCreateNode : LDNodeEditorNode
---@field modelId integer
---@field x number
---@field y number
---@field z number

local OBJECT_CREATE_NODE_TYPE = "core.object_create"

---@type LDNodeEditorNodeType
local objectCreateNode = {
    typeName = OBJECT_CREATE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorObjectCreateNode
        local newNode = {
            id = newNodeId,
            nodeType = OBJECT_CREATE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorObjectCreateNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local handle = ObjectOp.create(modelId, x, y, z)
        return {1, handle}
    end
}

return objectCreateNode
