...
---@class LDNodeEditorObjectSetProofsNode : LDNodeEditorNode
---@field self_ integer
...
local OBJECT_SET_PROOFS_NODE_TYPE = "core.object_set_proofs"

---@type LDNodeEditorNodeType
local objectSetProofsNode = {
    typeName = OBJECT_SET_PROOFS_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorObjectSetProofsNode
        local newNode = {
            id = newNodeId,
            nodeType = OBJECT_SET_PROOFS_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorObjectSetProofsNode
    ...
    run = function(editor, node, inputValues)
		...
		ObjectOp.setProofs(self_, bulletProof, fireProof, explosionProof, collisionProof, meleeProof)
        return {1}
    end
}

return objectSetProofsNode
