...
---@class LDNodeEditorTaskCarTempActionNode : LDNodeEditorNode
---@field char integer
---@field vehicle integer
---@field actionId integer
---@field time integer

local TASK_CAR_TEMP_ACTION_NODE_TYPE = "core.task_car_temp_action"

---@type LDNodeEditorNodeType
local taskCarTempActionNode = {
    typeName = TASK_CAR_TEMP_ACTION_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskCarTempActionNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_CAR_TEMP_ACTION_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskCarTempActionNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[4], "actionId", function ()
            node.actionId = editor.dataTypes[node.inputs[4].type].drawEditValue(node.actionId, "##actionIdEdit", fontScale * 100)
        end);
