...
---@class LDNodeEditorTaskFollowFootstepsNode : LDNodeEditorNode
---@field handle integer
---@field target integer

local TASK_FOLLOW_FOOTSTEPS_NODE_TYPE = "core.task_follow_footsteps"

---@type LDNodeEditorNodeType
local taskFollowFootstepsNode = {
    typeName = TASK_FOLLOW_FOOTSTEPS_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskFollowFootstepsNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_FOLLOW_FOOTSTEPS_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskFollowFootstepsNode
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.followFootsteps(handle, target)
        return {1}
    end
}

return taskFollowFootstepsNode
