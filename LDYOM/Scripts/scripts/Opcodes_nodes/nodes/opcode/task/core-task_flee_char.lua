...
---@class LDNodeEditorTaskFleeCharNode : LDNodeEditorNode
---@field handle integer
---@field threat integer
---@field radius number
---@field time integer

local TASK_FLEE_CHAR_NODE_TYPE = "core.task_flee_char"

---@type LDNodeEditorNodeType
local taskFleeCharNode = {
    typeName = TASK_FLEE_CHAR_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskFleeCharNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_FLEE_CHAR_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskFleeCharNode
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.fleeChar(handle, threat, radius, math.floor(time))
        return {1}
    end
}

return taskFleeCharNode
