...
---@class LDNodeEditorTaskPauseNode : LDNodeEditorNode
---@field handle integer
---@field time integer

local TASK_PAUSE_NODE_TYPE = "core.task_pause"

---@type LDNodeEditorNodeType
local taskPauseNode = {
    typeName = TASK_PAUSE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskPauseNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_PAUSE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskPauseNode
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.pause(handle, math.floor(time))
        return {1}
    end
}

return taskPauseNode
