...
---@class LDNodeEditorTaskPlayAnimNonInterruptableNode : LDNodeEditorNode
...
local TASK_PLAY_ANIM_NON_INTERRUPTABLE_NODE_TYPE = "core.task_play_anim_non_interruptable"

---@type LDNodeEditorNodeType
local taskPlayAnimNonInterruptableNode = {
    typeName = TASK_PLAY_ANIM_NON_INTERRUPTABLE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskPlayAnimNonInterruptableNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_PLAY_ANIM_NON_INTERRUPTABLE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.playAnimNonInterruptable(handle, animationName, animationFile, frameDelta, loop, lockX, lockY, lockF, math.floor(time))
        return {1}
    end
}

return taskPlayAnimNonInterruptableNode
