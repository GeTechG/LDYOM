...
---@class LDNodeEditorTaskTiredNode : LDNodeEditorNode
---@field handle integer
---@field time integer

local TASK_TIRED_NODE_TYPE = "core.task_tired"

---@type LDNodeEditorNodeType
local taskTiredNode = {
    typeName = TASK_TIRED_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskTiredNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_TIRED_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskTiredNode
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.tired(handle, math.floor(time))
        return {1}
    end
}

return taskTiredNode
