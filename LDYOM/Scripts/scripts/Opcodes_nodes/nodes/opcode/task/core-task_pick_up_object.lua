...
---@class LDNodeEditorTaskPickUpObjectNode : LDNodeEditorNode
---@field char integer
...
local TASK_PICK_UP_OBJECT_NODE_TYPE = "core.task_pick_up_object"

---@type LDNodeEditorNodeType
local taskPickUpObjectNode = {
    typeName = TASK_PICK_UP_OBJECT_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskPickUpObjectNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_PICK_UP_OBJECT_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.pickUpObject(char, object, xOffset, yOffset, zOffset, math.floor(boneId), math.floor(_p7), animationName, animationFile, math.floor(time))
        return {1}
    end
}

return taskPickUpObjectNode
