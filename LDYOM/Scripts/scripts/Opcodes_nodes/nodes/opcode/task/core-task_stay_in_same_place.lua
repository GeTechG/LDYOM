...
---@class LDNodeEditorTaskStayInSamePlaceNode : LDNodeEditorNode
---@field handle integer
---@field state boolean

local TASK_STAY_IN_SAME_PLACE_NODE_TYPE = "core.task_stay_in_same_place"

---@type LDNodeEditorNodeType
local taskStayInSamePlaceNode = {
    typeName = TASK_STAY_IN_SAME_PLACE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskStayInSamePlaceNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_STAY_IN_SAME_PLACE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskStayInSamePlaceNode
    ...
    run = function(editor, context, node, inputValues)
		...
		TaskOp.stayInSamePlace(handle, state)
        return {1}
    end
}

return taskStayInSamePlaceNode
