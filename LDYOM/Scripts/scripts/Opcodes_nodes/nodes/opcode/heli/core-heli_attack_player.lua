...
---@class LDNodeEditorHeliAttackPlayerNode : LDNodeEditorNode
---@field self_ integer
---@field handle integer
---@field radius number

local HELI_ATTACK_PLAYER_NODE_TYPE = "core.heli_attack_player"

---@type LDNodeEditorNodeType
local heliAttackPlayerNode = {
    typeName = HELI_ATTACK_PLAYER_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorHeliAttackPlayerNode
        local newNode = {
            id = newNodeId,
            nodeType = HELI_ATTACK_PLAYER_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorHeliAttackPlayerNode
    ...
    run = function(editor, context, node, inputValues)
		...
		HeliOp.attackPlayer(self_, handle, radius)
        return {1}
    end
}

return heliAttackPlayerNode
