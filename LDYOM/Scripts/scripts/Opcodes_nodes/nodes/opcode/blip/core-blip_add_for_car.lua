...
---@class LDNodeEditorBlipAddForCarNode : LDNodeEditorNode
---@field vehicle integer

local BLIP_ADD_FOR_CAR_NODE_TYPE = "core.blip_add_for_car"

---@type LDNodeEditorNodeType
local blipAddForCarNode = {
    typeName = BLIP_ADD_FOR_CAR_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorBlipAddForCarNode
        local newNode = {
            id = newNodeId,
            nodeType = BLIP_ADD_FOR_CAR_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorBlipAddForCarNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local handle = BlipOp.addForCar(vehicle)
        return {1, handle}
    end
}

return blipAddForCarNode
