...
---@class LDNodeEditorCameraAttachToVehicleNode : LDNodeEditorNode
...
local CAMERA_ATTACH_TO_VEHICLE_NODE_TYPE = "core.camera_attach_to_vehicle"

---@type LDNodeEditorNodeType
local cameraAttachToVehicleNode = {
    typeName = CAMERA_ATTACH_TO_VEHICLE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCameraAttachToVehicleNode
        local newNode = {
            id = newNodeId,
            nodeType = CAMERA_ATTACH_TO_VEHICLE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCameraAttachToVehicleNode
    ...
    run = function(editor, context, node, inputValues)
		...
		CameraOp.attachToVehicle(handle, xOffset, yOffset, zOffset, xRotation, yRotation, zRotation, tilt, switchStyle)
        return {1}
    end
}

return cameraAttachToVehicleNode
