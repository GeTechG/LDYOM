...
---@class LDNodeEditorCameraPointAtPointNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number
---@field switchStyle integer

local CAMERA_POINT_AT_POINT_NODE_TYPE = "core.camera_point_at_point"

---@type LDNodeEditorNodeType
local cameraPointAtPointNode = {
    typeName = CAMERA_POINT_AT_POINT_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCameraPointAtPointNode
        local newNode = {
            id = newNodeId,
            nodeType = CAMERA_POINT_AT_POINT_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCameraPointAtPointNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[5], "switchStyle", function ()
            local name = "<Not set>";
    ...
    run = function(editor, node, inputValues)
		...
		local switchStyle = inputValues[5] or node.switchStyle
		CameraOp.pointAtPoint(x, y, z, switchStyle)
        return {1}
    end
}

return cameraPointAtPointNode
