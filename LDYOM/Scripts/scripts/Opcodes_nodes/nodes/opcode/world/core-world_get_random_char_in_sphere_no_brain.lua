...
---@class LDNodeEditorWorldGetRandomCharInSphereNoBrainNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number
---@field radius number

local WORLD_GET_RANDOM_CHAR_IN_SPHERE_NO_BRAIN_NODE_TYPE = "core.world_get_random_char_in_sphere_no_brain"

---@type LDNodeEditorNodeType
local worldGetRandomCharInSphereNoBrainNode = {
    typeName = WORLD_GET_RANDOM_CHAR_IN_SPHERE_NO_BRAIN_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldGetRandomCharInSphereNoBrainNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_GET_RANDOM_CHAR_IN_SPHERE_NO_BRAIN_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldGetRandomCharInSphereNoBrainNode
    ...
    run = function(editor, node, inputValues)
		...
		local handle = WorldOp.getRandomCharInSphereNoBrain(x, y, z, radius)
        return {1, handle}
    end
}

return worldGetRandomCharInSphereNoBrainNode
