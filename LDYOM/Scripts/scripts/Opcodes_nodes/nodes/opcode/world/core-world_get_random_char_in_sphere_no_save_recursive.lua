...
local worldGetRandomCharInSphereNoSaveRecursiveNode = {
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[6], "skipDead", function ()
            node.skipDead = editor.dataTypes[node.inputs[6].type].drawEditValue(node.skipDead, "##skipDeadEdit", fontScale * 100)
        end);
    ...
    run = function(editor, context, node, inputValues)
		...
		local skipDead = inputValues[6] or node.skipDead
