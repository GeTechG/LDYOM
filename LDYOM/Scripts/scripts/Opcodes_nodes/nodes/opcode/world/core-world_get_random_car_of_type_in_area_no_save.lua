...
---@class LDNodeEditorWorldGetRandomCarOfTypeInAreaNoSaveNode : LDNodeEditorNode
...
local WORLD_GET_RANDOM_CAR_OF_TYPE_IN_AREA_NO_SAVE_NODE_TYPE = "core.world_get_random_car_of_type_in_area_no_save"

---@type LDNodeEditorNodeType
local worldGetRandomCarOfTypeInAreaNoSaveNode = {
    typeName = WORLD_GET_RANDOM_CAR_OF_TYPE_IN_AREA_NO_SAVE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldGetRandomCarOfTypeInAreaNoSaveNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_GET_RANDOM_CAR_OF_TYPE_IN_AREA_NO_SAVE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldGetRandomCarOfTypeInAreaNoSaveNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local handle = WorldOp.getRandomCarOfTypeInAreaNoSave(leftBottomX, leftBottomY, rightTopX, rightTopY, modelId)
        return {1, handle}
    end
}

return worldGetRandomCarOfTypeInAreaNoSaveNode
