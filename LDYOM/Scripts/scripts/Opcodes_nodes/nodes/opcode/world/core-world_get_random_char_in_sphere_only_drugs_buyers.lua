...
---@class LDNodeEditorWorldGetRandomCharInSphereOnlyDrugsBuyersNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number
---@field radius number

local WORLD_GET_RANDOM_CHAR_IN_SPHERE_ONLY_DRUGS_BUYERS_NODE_TYPE = "core.world_get_random_char_in_sphere_only_drugs_buyers"

---@type LDNodeEditorNodeType
local worldGetRandomCharInSphereOnlyDrugsBuyersNode = {
    typeName = WORLD_GET_RANDOM_CHAR_IN_SPHERE_ONLY_DRUGS_BUYERS_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldGetRandomCharInSphereOnlyDrugsBuyersNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_GET_RANDOM_CHAR_IN_SPHERE_ONLY_DRUGS_BUYERS_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldGetRandomCharInSphereOnlyDrugsBuyersNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local handle = WorldOp.getRandomCharInSphereOnlyDrugsBuyers(x, y, z, radius)
        return {1, handle}
    end
}

return worldGetRandomCharInSphereOnlyDrugsBuyersNode
