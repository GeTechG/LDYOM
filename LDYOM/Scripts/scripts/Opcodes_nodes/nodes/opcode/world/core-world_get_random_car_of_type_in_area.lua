...
---@class LDNodeEditorWorldGetRandomCarOfTypeInAreaNode : LDNodeEditorNode
...
local WORLD_GET_RANDOM_CAR_OF_TYPE_IN_AREA_NODE_TYPE = "core.world_get_random_car_of_type_in_area"

---@type LDNodeEditorNodeType
local worldGetRandomCarOfTypeInAreaNode = {
    typeName = WORLD_GET_RANDOM_CAR_OF_TYPE_IN_AREA_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldGetRandomCarOfTypeInAreaNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_GET_RANDOM_CAR_OF_TYPE_IN_AREA_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldGetRandomCarOfTypeInAreaNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local handle = WorldOp.getRandomCarOfTypeInArea(leftBottomX, leftBottomY, rightTopX, rightTopY, modelId)
        return {1, handle}
    end
}

return worldGetRandomCarOfTypeInAreaNode
