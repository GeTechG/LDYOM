...
---@class LDNodeEditorWorldSetTagStatusInAreaNode : LDNodeEditorNode
...
local WORLD_SET_TAG_STATUS_IN_AREA_NODE_TYPE = "core.world_set_tag_status_in_area"

---@type LDNodeEditorNodeType
local worldSetTagStatusInAreaNode = {
    typeName = WORLD_SET_TAG_STATUS_IN_AREA_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldSetTagStatusInAreaNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_SET_TAG_STATUS_IN_AREA_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldSetTagStatusInAreaNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[6], "percent", function ()
            node.percent = editor.dataTypes[node.inputs[6].type].drawEditValue(node.percent, "##percentEdit", fontScale * 100)
        end);
...
return worldSetTagStatusInAreaNode
