...
---@class LDNodeEditorWorldGetNumberOfFiresInAreaNode : LDNodeEditorNode
...
local WORLD_GET_NUMBER_OF_FIRES_IN_AREA_NODE_TYPE = "core.world_get_number_of_fires_in_area"

---@type LDNodeEditorNodeType
local worldGetNumberOfFiresInAreaNode = {
    typeName = WORLD_GET_NUMBER_OF_FIRES_IN_AREA_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldGetNumberOfFiresInAreaNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_GET_NUMBER_OF_FIRES_IN_AREA_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldGetNumberOfFiresInAreaNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local numFires = WorldOp.getNumberOfFiresInArea(leftBottomX, leftBottomY, leftBottomZ, rightTopX, rightTopY, rightTopZ)
        return {1, numFires}
    end
}

return worldGetNumberOfFiresInAreaNode
