...
---@class LDNodeEditorZoneSetNoCopsNode : LDNodeEditorNode
---@field zone string
---@field state boolean

local ZONE_SET_NO_COPS_NODE_TYPE = "core.zone_set_no_cops"

---@type LDNodeEditorNodeType
local zoneSetNoCopsNode = {
    typeName = ZONE_SET_NO_COPS_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorZoneSetNoCopsNode
        local newNode = {
            id = newNodeId,
            nodeType = ZONE_SET_NO_COPS_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorZoneSetNoCopsNode
    ...
    run = function(editor, context, node, inputValues)
		...
		ZoneOp.setNoCops(zone, state)
        return {1}
    end
}

return zoneSetNoCopsNode
