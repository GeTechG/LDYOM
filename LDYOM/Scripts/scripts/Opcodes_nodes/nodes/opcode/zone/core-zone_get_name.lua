...
---@class LDNodeEditorZoneGetNameNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number

local ZONE_GET_NAME_NODE_TYPE = "core.zone_get_name"

---@type LDNodeEditorNodeType
local zoneGetNameNode = {
    typeName = ZONE_GET_NAME_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorZoneGetNameNode
        local newNode = {
            id = newNodeId,
            nodeType = ZONE_GET_NAME_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorZoneGetNameNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultOutput(editor, ctx, builder, node.outputs[2], "name");
    ...
    run = function(editor, context, node, inputValues)
		...
		local name = ZoneOp.getName(x, y, z)
        return {1, name}
    end
}

return zoneGetNameNode
