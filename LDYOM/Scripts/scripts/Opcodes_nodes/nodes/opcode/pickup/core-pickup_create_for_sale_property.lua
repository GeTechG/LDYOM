...
---@class LDNodeEditorPickupCreateForSalePropertyNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number
---@field price integer
...
local PICKUP_CREATE_FOR_SALE_PROPERTY_NODE_TYPE = "core.pickup_create_for_sale_property"

---@type LDNodeEditorNodeType
local pickupCreateForSalePropertyNode = {
    typeName = PICKUP_CREATE_FOR_SALE_PROPERTY_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorPickupCreateForSalePropertyNode
        local newNode = {
            id = newNodeId,
            nodeType = PICKUP_CREATE_FOR_SALE_PROPERTY_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorPickupCreateForSalePropertyNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[5], "price", function ()
            node.price = editor.dataTypes[node.inputs[5].type].drawEditValue(node.price, "##priceEdit", fontScale * 100)
        end);

		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[6], "message", function ()
            node.message = editor.dataTypes[node.inputs[6].type].drawEditValue(node.message, "##messageEdit", fontScale * 100)
        end);
...
return pickupCreateForSalePropertyNode
