...
---@class LDNodeEditorPickupCreateHorseshoeNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number

local PICKUP_CREATE_HORSESHOE_NODE_TYPE = "core.pickup_create_horseshoe"

---@type LDNodeEditorNodeType
local pickupCreateHorseshoeNode = {
    typeName = PICKUP_CREATE_HORSESHOE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorPickupCreateHorseshoeNode
        local newNode = {
            id = newNodeId,
            nodeType = PICKUP_CREATE_HORSESHOE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorPickupCreateHorseshoeNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local handle = PickupOp.createHorseshoe(x, y, z)
        return {1, handle}
    end
}

return pickupCreateHorseshoeNode
