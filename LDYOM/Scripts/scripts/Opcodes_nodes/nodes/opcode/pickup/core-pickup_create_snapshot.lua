...
---@class LDNodeEditorPickupCreateSnapshotNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number

local PICKUP_CREATE_SNAPSHOT_NODE_TYPE = "core.pickup_create_snapshot"

---@type LDNodeEditorNodeType
local pickupCreateSnapshotNode = {
    typeName = PICKUP_CREATE_SNAPSHOT_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorPickupCreateSnapshotNode
        local newNode = {
            id = newNodeId,
            nodeType = PICKUP_CREATE_SNAPSHOT_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorPickupCreateSnapshotNode
    ...
    run = function(editor, node, inputValues)
		...
		local handle = PickupOp.createSnapshot(x, y, z)
        return {1, handle}
    end
}

return pickupCreateSnapshotNode
