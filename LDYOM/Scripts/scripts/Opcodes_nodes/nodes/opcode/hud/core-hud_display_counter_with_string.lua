...
---@class LDNodeEditorHudDisplayCounterWithStringNode : LDNodeEditorNode
---@field counter integer
---@field display integer
---@field text string

local HUD_DISPLAY_COUNTER_WITH_STRING_NODE_TYPE = "core.hud_display_counter_with_string"

---@type LDNodeEditorNodeType
local hudDisplayCounterWithStringNode = {
    typeName = HUD_DISPLAY_COUNTER_WITH_STRING_NODE_TYPE,
    category = "opcode.hud",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorHudDisplayCounterWithStringNode
        local newNode = {
            id = newNodeId,
            nodeType = HUD_DISPLAY_COUNTER_WITH_STRING_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorHudDisplayCounterWithStringNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[4], "text", function ()
            node.text = editor.dataTypes[node.inputs[4].type].drawEditValue(node.text, "##textEdit", fontScale * 100)
        end);
...
return hudDisplayCounterWithStringNode
