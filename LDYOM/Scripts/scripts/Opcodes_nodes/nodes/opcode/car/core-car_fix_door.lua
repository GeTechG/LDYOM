...
---@class LDNodeEditorCarFixDoorNode : LDNodeEditorNode
---@field self_ integer
---@field door integer

local CAR_FIX_DOOR_NODE_TYPE = "core.car_fix_door"

---@type LDNodeEditorNodeType
local carFixDoorNode = {
    typeName = CAR_FIX_DOOR_NODE_TYPE,
    category = "opcode.car",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCarFixDoorNode
        local newNode = {
            id = newNodeId,
            nodeType = CAR_FIX_DOOR_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCarFixDoorNode
    ...
    run = function(editor, context, node, inputValues)
		...
		CarOp.fixDoor(self_, door)
        return {1}
    end
}

return carFixDoorNode
