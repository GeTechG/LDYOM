...
---@class LDNodeEditorCarLocate2DNode : LDNodeEditorNode
---@field self_ integer
---@field x number
---@field y number
...
local CAR_LOCATE2_D_NODE_TYPE = "core.car_locate2_d"

---@type LDNodeEditorNodeType
local carLocate2DNode = {
    typeName = CAR_LOCATE2_D_NODE_TYPE,
    category = "opcode.car",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCarLocate2DNode
        local newNode = {
            id = newNodeId,
            nodeType = CAR_LOCATE2_D_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCarLocate2DNode
    ...
    run = function(editor, context, node, inputValues)
		...
		local result = CarOp.locate2D(self_, x, y, xRadius, yRadius, drawSphere)
        return {result}
    end
}

return carLocate2DNode
