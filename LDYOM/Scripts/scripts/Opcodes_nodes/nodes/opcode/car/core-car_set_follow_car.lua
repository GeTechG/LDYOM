...
---@class LDNodeEditorCarSetFollowCarNode : LDNodeEditorNode
---@field self_ integer
---@field handle integer
---@field radius number

local CAR_SET_FOLLOW_CAR_NODE_TYPE = "core.car_set_follow_car"

---@type LDNodeEditorNodeType
local carSetFollowCarNode = {
    typeName = CAR_SET_FOLLOW_CAR_NODE_TYPE,
    category = "opcode.car",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCarSetFollowCarNode
        local newNode = {
            id = newNodeId,
            nodeType = CAR_SET_FOLLOW_CAR_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    run = function(editor, node, inputValues)
		...
		CarOp.setFollowCar(self_, handle, radius)
        return {1}
    end
}

return carSetFollowCarNode
