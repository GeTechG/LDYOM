...
---@class LDNodeEditorCarCreateNode : LDNodeEditorNode
---@field modelId integer
---@field x number
---@field y number
---@field z number

local CAR_CREATE_NODE_TYPE = "core.car_create"

---@type LDNodeEditorNodeType
local carCreateNode = {
    typeName = CAR_CREATE_NODE_TYPE,
    category = "opcode.car",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCarCreateNode
        local newNode = {
            id = newNodeId,
            nodeType = CAR_CREATE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCarCreateNode
    ...
    run = function(editor, node, inputValues)
		...
		local handle = CarOp.create(modelId, x, y, z)
        return {1, handle}
    end
}

return carCreateNode
