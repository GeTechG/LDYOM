...
---@class LDNodeEditorCarSetCoordinatesNoOffsetNode : LDNodeEditorNode
---@field self_ integer
---@field x number
---@field y number
---@field z number

local CAR_SET_COORDINATES_NO_OFFSET_NODE_TYPE = "core.car_set_coordinates_no_offset"

---@type LDNodeEditorNodeType
local carSetCoordinatesNoOffsetNode = {
    typeName = CAR_SET_COORDINATES_NO_OFFSET_NODE_TYPE,
    category = "opcode.car",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCarSetCoordinatesNoOffsetNode
        local newNode = {
            id = newNodeId,
            nodeType = CAR_SET_COORDINATES_NO_OFFSET_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCarSetCoordinatesNoOffsetNode
    ...
    run = function(editor, context, node, inputValues)
		...
		CarOp.setCoordinatesNoOffset(self_, x, y, z)
        return {1}
    end
}

return carSetCoordinatesNoOffsetNode
