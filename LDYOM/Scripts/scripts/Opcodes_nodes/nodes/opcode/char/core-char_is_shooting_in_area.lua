...
local charIsShootingInAreaNode = {
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[4], "topRightX", function ()
            node.topRightX = editor.dataTypes[node.inputs[4].type].drawEditValue(node.topRightX, "##topRightXEdit", fontScale * 100)
        end);

		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[5], "topRightY", function ()
            node.topRightY = editor.dataTypes[node.inputs[5].type].drawEditValue(node.topRightY, "##topRightYEdit", fontScale * 100)
        end);
    ...
    run = function(editor, context, node, inputValues)
		...
		local topRightX = inputValues[4] or node.topRightX
		local topRightY = inputValues[5] or node.topRightY
