...
---@class LDNodeEditorCharLocateAnyMeansCar3DNode : LDNodeEditorNode
---@field self_ integer
...
local CHAR_LOCATE_ANY_MEANS_CAR3_D_NODE_TYPE = "core.char_locate_any_means_car3_d"

---@type LDNodeEditorNodeType
local charLocateAnyMeansCar3DNode = {
    typeName = CHAR_LOCATE_ANY_MEANS_CAR3_D_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCharLocateAnyMeansCar3DNode
        local newNode = {
            id = newNodeId,
            nodeType = CHAR_LOCATE_ANY_MEANS_CAR3_D_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    run = function(editor, context, node, inputValues)
		...
		local result = CharOp.locateAnyMeansCar3D(self_, vehicle, xRadius, yRadius, zRadius, drawSphere)
        return {result}
    end
}

return charLocateAnyMeansCar3DNode
