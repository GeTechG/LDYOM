...
---@class DeleteAudioEvent
---@field static table
---@field audioUuid ffi.cdata*

---@type BaseNode | DeleteAudioEvent
local DeleteAudioEvent = class("DeleteAudioEvent", ed.BaseNode)
DeleteAudioEvent.static.name_ = "core.nodes.delete_audio_event";
DeleteAudioEvent.static.type = ed.NodeType.Event;

---@param id integer
function DeleteAudioEvent:initialize(id)
...
function DeleteAudioEvent:draw()
...
function DeleteAudioEvent:execute(scene, tasklist, context)
    context:callNodes(scene, tasklist, self.id + 2);
end


---@param scene Scene
global_data.signals.onAudioDelete[#global_data.signals.onAudioDelete + 1] = function(scene, tasklist, audio_uuid)
    local context = global_data.ed_contexts[scene:getId()];
    for _, node in pairs(context.nodes) do
        if node.class == DeleteAudioEvent then
...
NodeEditor.registerNode("core.categories.audio", DeleteAudioEvent);