...
---@class LDNodeEditorDestroyParticleNode : LDNodeEditorNode
---@field sceneId integer
---@field entityUuid string

local DESTROY_PARTICLE_NODE_TYPE = "core.destroy_particle"

---@type LDNodeEditorNodeType
local destroyParticleNode = {
    typeName = DESTROY_PARTICLE_NODE_TYPE,
    category = "particle",
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorDestroyParticleNode
        local newNode = {
            id = newNodeId,
            nodeType = DESTROY_PARTICLE_NODE_TYPE,
            buffer = "",
            inputs = {
                {
                    id = getPinId(),
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorDestroyParticleNode
    ...
    draw = function(editor, ctx, node, builder)
        ...
        if scene then
            local particles = scene:getParticles();
            local indexParticle = ld.indexByUuid(particles, node.entityUuid);

            ImGui.Text(ld.loc.get("entities.particle"));
            ImGui.SameLine(0, -1);
            if ImGui.Button((indexParticle ~= -1 and particles[indexParticle + 1]:getName()) or "<None>", ImVec2.new()) then
                NodeEditor.Suspend();
            ...
            if autoBindRequireFields then
                if node.entityUuid == "" and #particles > 0 then
                    node.entityUuid = tostring(particles[#particles]:getUuid());
                end
            end
                ...
                if ImGui.BeginPopup("##entityUuidSelect", 0) then
                    if ImGui.BeginListBox("##entityUuidSelectChild", ImVec2.new(200 * fontScale, 100 * fontScale)) then
                        for i, particle in ipairs(particles) do
                            if ImGui.Selectable(particle:getName(), node.entityUuid == tostring(particle:getUuid()), 0, ImVec2.new()) then
                                node.entityUuid = tostring(particle:getUuid());
                                ImGui.CloseCurrentPopup();
                            end
                        end
                        ImGui.EndListBox();
    ...
    run = function(editor, context, node, inputValues)
        ...
        if node.entityUuid == "" then
            error("Particle is not set!")
        end
        local particles = ld.projectsService():getCurrentProject():getScenes()[node.sceneId]:getParticles();
        local indexParticle = ld.indexByUuid(particles, node.entityUuid);
        if indexParticle == -1 then
