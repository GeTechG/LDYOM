...
---@type BaseNode | SpawnParticleEvent
local SpawnParticleEvent = class("SpawnParticleEvent", ed.BaseNode)
SpawnParticleEvent.static.name_ = "core.nodes.spawn_particle_event";
SpawnParticleEvent.static.type = ed.NodeType.Event;

---@param id integer
function SpawnParticleEvent:initialize(id)
...
function SpawnParticleEvent:draw()
...
function SpawnParticleEvent:execute(scene, tasklist, context)
    context:callNodes(scene, tasklist, self.id + 2);
end


---@param scene Scene
global_data.signals.onParticleSpawn[#global_data.signals.onParticleSpawn + 1] = function(scene, tasklist, particle_uuid)
    local context = global_data.ed_contexts[scene:getId()];
    for _, node in pairs(context.nodes) do
        if node.class == SpawnParticleEvent then
...
NodeEditor.registerNode("core.categories.particle", SpawnParticleEvent);