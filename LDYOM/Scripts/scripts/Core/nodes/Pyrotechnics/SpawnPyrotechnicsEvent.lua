...
---@class SpawnPyrotechnicsEvent
---@field static table
---@field pyrotechnicsUuid ffi.cdata*

---@type BaseNode | SpawnPyrotechnicsEvent
local SpawnPyrotechnicsEvent = class("SpawnPyrotechnicsEvent", ed.BaseNode)
SpawnPyrotechnicsEvent.static.name_ = "core.nodes.spawn_pyrotechnics_event";
SpawnPyrotechnicsEvent.static.type = ed.NodeType.Event;

---@param id integer
function SpawnPyrotechnicsEvent:initialize(id)
...
function SpawnPyrotechnicsEvent:draw()
...
function SpawnPyrotechnicsEvent:execute(scene, tasklist, context)
    context:callNodes(scene, tasklist, self.id + 2);
end


---@param scene Scene
global_data.signals.onPyrotechnicsSpawn[#global_data.signals.onPyrotechnicsSpawn + 1] = function(scene, tasklist, pyrotechnics_uuid)
    local context = global_data.ed_contexts[scene:getId()];
    for _, node in pairs(context.nodes) do
        if node.class == SpawnPyrotechnicsEvent then
...
NodeEditor.registerNode("core.categories.pyrotechnics", SpawnPyrotechnicsEvent);