...
---@class DeletePyrotechnics
---@field static table

---@type BaseNode | DeletePyrotechnics
local DeletePyrotechnics = class("DeletePyrotechnics", ed.BaseNode)
DeletePyrotechnics.static.name_ = "core.nodes.delete_pyrotechnics";
DeletePyrotechnics.static.type = ed.NodeType.Function;

---@param id integer
function DeletePyrotechnics:initialize(id)
...
function DeletePyrotechnics:draw()
...
function DeletePyrotechnics:execute(scene, tasklist, context)
    local pyrotechnicss = ProjectsService.getInstance():getCurrentProject():getCurrentScene():getPyrotechnics();
	local indexPyrotechnics = indexByUuid(pyrotechnicss, self.pyrotechnicsUuid);
    assert(indexPyrotechnics ~= -1, "Pyrotechnics not selected");
    ---@type Pyrotechnics
    local pyrotechnics = pyrotechnicss[indexPyrotechnics+1];
    pyrotechnics:deleteProjectEntity();
    
    context:callNodes(scene, tasklist, self.id + 3);
end

NodeEditor.registerNode("core.categories.pyrotechnics", DeletePyrotechnics);