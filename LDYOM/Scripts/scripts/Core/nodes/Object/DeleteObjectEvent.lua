...
---@type BaseNode | DeleteObjectEvent
local DeleteObjectEvent = class("DeleteObjectEvent", ed.BaseNode)
DeleteObjectEvent.static.name_ = "core.nodes.delete_object_event";
DeleteObjectEvent.static.type = ed.NodeType.Event;

---@param id integer
function DeleteObjectEvent:initialize(id)
...
function DeleteObjectEvent:draw()
...
function DeleteObjectEvent:execute(scene, tasklist, context)
    context:callNodes(scene, tasklist, self.id + 2);
end


---@param scene Scene
global_data.signals.onObjectDelete[#global_data.signals.onObjectDelete + 1] = function(scene, tasklist, object_uuid)
    local context = global_data.ed_contexts[scene:getId()];
    for _, node in pairs(context.nodes) do
        if node.class == DeleteObjectEvent then
...
NodeEditor.registerNode("core.categories.object", DeleteObjectEvent);