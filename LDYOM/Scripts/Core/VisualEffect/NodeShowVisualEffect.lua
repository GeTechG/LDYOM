...
Node = bitser.registerClass(class("NodeShowVisualEffect", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeShowVisualEffect");
...
function Node:draw()
	...
	local names = ldyom.namesVisualEffects;
	imgui.Text(ldyom.langt("visualEffect"));
...
function Node:play(data, mission)
	local effect = self:getPinValue(self.id+2,data,mission)[0];
	ldyom.setLastNode(self.id);
	assert(effect < #mission.list_visualEffects,"The ID of the effect exceeds the number of effects.");
	ldyom.showVisualEffect(mission.list_visualEffects[effect+1]);
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeApperVisualEffect");
		local name_node = v["class"]["static"]["name"];
		if name == name_node and v["Pins"][v.id+1].value[0] == effect then
...
ldyom.nodeEditor.addNodeClass("VisualEffect",Node);