...
Node = bitser.registerClass(class("NodeHideVisualEffect", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeHideVisualEffect");
...
function Node:play(data, mission)
	...
	ldyom.hideVisualEffect(mission.list_visualEffects[effect+1]);
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeDisapperVisualEffect");
