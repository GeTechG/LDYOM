...
Node = bitser.registerClass(class("NodeGetPosVehicle", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeGetPosVehicle");
...
function Node:play(data, mission)
	...
	local car = self:getPinValue(self.id+1,data,mission)[0];
	assert(callOpcode(0x056E, {{car,"int"}}), "Not found vehicle");
	ldyom.setLastNode(self.id);
	
	callOpcode(0x00AA, {{car,"int"}, {self.Pins[self.id+2].value,"float*"}, {self.Pins[self.id+3].value,"float*"}, {self.Pins[self.id+4].value,"float*"}});
end
