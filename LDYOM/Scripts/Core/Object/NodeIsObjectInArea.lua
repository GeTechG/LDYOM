...
Node = bitser.registerClass(class("NodeIsObjectInArea", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("fork")..' '..ldyom.langt("CoreNodeIsObjectInArea");
...
function Node:play(data, mission)
	...
	if self.three[0] then
		result[0] = callOpcode(0x04EA, {{object,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
	else
		result[0] = callOpcode(0x04E9, {{object,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
	end
end
