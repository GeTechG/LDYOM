...
Node = bitser.registerClass(class("NodeGetPosObject", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeGetPosObject");
...
function Node:play(data, mission)
	...
	local obj = self:getPinValue(self.id+1,data,mission)[0];
	assert(callOpcode(0x03CA, {{obj,"int"}}), "Not found object");
	ldyom.setLastNode(self.id);
	
	callOpcode(0x01BB, {{obj,"int"}, {self.Pins[self.id+2].value,"float*"}, {self.Pins[self.id+3].value,"float*"}, {self.Pins[self.id+4].value,"float*"}});
end
