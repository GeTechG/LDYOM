...
Node = bitser.registerClass(class("NodeHideObject", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeHideObject");
...
function Node:play(data, mission)
	...
	assert(object < #mission.list_objects,"The ID of the object exceeds the number of objects.");
	mission.list_objects[object+1]:removeMissionObject();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeDisapperObject");
		local name_node = v["class"]["static"]["name"];
		if name == name_node and v["Pins"][v.id+1].value[0] == object then
