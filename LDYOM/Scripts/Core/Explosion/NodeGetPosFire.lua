...
Node = bitser.registerClass(class("NodeGetPosFire", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeGetPosFire");
...
function Node:play(data, mission)
	...
	local fire = self:getPinValue(self.id+1,data,mission)[0];
	assert(callOpcode(0x0973, {{fire,"int"}}), "Not found fire");
	ldyom.setLastNode(self.id);
	
	callOpcode(0x06F5, {{fire,"int"}, {self.Pins[self.id+2].value,"float*"}, {self.Pins[self.id+3].value,"float*"}, {self.Pins[self.id+4].value,"float*"}});
end
