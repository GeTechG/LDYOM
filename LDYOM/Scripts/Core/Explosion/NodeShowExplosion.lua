...
Node = bitser.registerClass(class("NodeShowExplosion", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeShowExplosion");
...
function Node:play(data, mission)
	...
	mission.list_explosions[explosion+1]:updateMissionExplosion();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeApperExplosion");
