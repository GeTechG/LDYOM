...
Node = bitser.registerClass(class("NodeHideTrain", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeHideTrain");
...
function Node:play(data, mission)
	local train = self:getPinValue(self.id+2,data,mission)[0];
	ldyom.setLastNode(self.id);
	assert(train < #mission.list_trains,"The ID of the train exceeds the number of trains.");
	mission.list_trains[train+1]:removeMissionTrain();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeDisapperTrain");
		local name_node = v["class"]["static"]["name"];
		if name == name_node and v["Pins"][v.id+1].value[0] == train then
