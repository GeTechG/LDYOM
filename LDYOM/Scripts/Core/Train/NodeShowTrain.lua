...
Node = bitser.registerClass(class("NodeShowTrain", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeShowTrain");
...
function Node:play(data, mission)
	...
	mission.list_trains[train+1]:updateMissionTrain();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeApperTrain");
