...
Node = bitser.registerClass(class("NodeHideParticle", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeHideParticle");
...
function Node:play(data, mission)
	...
	mission.list_particles[particle+1]:removeMissionParticle();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeDisapperParticle");
