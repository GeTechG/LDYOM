...
Node = bitser.registerClass(class("NodeShowParticle", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeShowParticle");
...
function Node:play(data, mission)
	local particle = self:getPinValue(self.id+2,data,mission)[0];
	ldyom.setLastNode(self.id);
	assert(particle < #mission.list_particles,"The ID of the particle exceeds the number of particles.");
	mission.list_particles[particle+1]:updateMissionParticle(mission);
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeApperParticle");
		local name_node = v["class"]["static"]["name"];
		if name == name_node and v["Pins"][v.id+1].value[0] == particle then
