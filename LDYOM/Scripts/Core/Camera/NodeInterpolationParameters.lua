...
Node = bitser.registerClass(class("NodeInterpolationParameters", BaseNode));

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeInterpolationParameters");
...
function Node:initialize(id)
	...
	self.Pins = {
		...
		[self.id+2] = BasePin:new(self.id+2,imgui.imnodes.PinType.number, 0, ffi.new("float[1]",0.0)),
		[self.id+3] = BasePin:new(self.id+3,imgui.imnodes.PinType.number, 0, ffi.new("float[1]",0.0)),
...
function Node:draw()
	...
	imgui.Text(ldyom.langt("interpolation"));
...
function Node:play(data, mission)
	local interpolation = self:getPinValue(self.id+2,data,mission)[0];
	local _time = self:getPinValue(self.id+3,data,mission)[0]*1000;
	ldyom.setLastNode(self.id);
	
	wait(0)
	callOpcode(0x0460, {{interpolation,"float"}, {math.floor(_time),"int"}});
