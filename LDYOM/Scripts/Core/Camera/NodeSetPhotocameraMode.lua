...
Node = bitser.registerClass(class("NodeSetPhotocameraMode", BaseNode));

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeSetPhotocameraMode");
...
function Node:draw()
	...
	imgui.ToggleButton(ldyom.langt("value"), self.Pins[self.id+2].value);
	imgui.imnodes.EndInputAttribute();
...
function Node:play(data, mission)
	local toggle = self:getPinValue(self.id+2,data,mission)[0];
	ldyom.setLastNode(self.id);

	callOpcode(0x0A2F, {{toggle, "bool"}});
