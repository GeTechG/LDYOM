...
function attachToVehicleLookChar()
	ldyom.attachCameraToEntity(3, currNodeStaticCamera.Pins[currNodeStaticCamera.id+2].value[0], currNodeStaticCamera.Pins[currNodeStaticCamera.id+6].value[0], currNodeStaticCamera.Pins[currNodeStaticCamera.id+3].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+4].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+5].value, nil, nil, nil);
end
...
function Node:play(data, mission)
	...
	local ped2 = self:getPinValue(self.id+6,data,mission)[0];
	...
	local ped2Ref = getPedRef(mission.list_actors[ped2+1].missionPed);

	callOpcode(0x067E, {{pedRef,"int"}, {x,"float"}, {y,"float"}, {z,"float"}, {ped2Ref,"int"}, {0.0,"float"}, {fif(movecam,1,2),"int"}});
