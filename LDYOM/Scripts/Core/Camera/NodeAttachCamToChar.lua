...
Node = bitser.registerClass(class("NodeAttachCamToChar", BaseNode));

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeAttachCamToChar");
...
function attachToChar()
	ldyom.attachCameraToEntity(1, currNodeStaticCamera.Pins[currNodeStaticCamera.id+2].value[0], 0, currNodeStaticCamera.Pins[currNodeStaticCamera.id+3].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+4].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+5].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+6].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+7].value, currNodeStaticCamera.Pins[currNodeStaticCamera.id+8].value);
end
...
function Node:play(data, mission)
	...
	local pedRef = getPedRef(mission.list_actors[actor+1].missionPed);

	if not self.pointPos then
		rot_x, rot_y, rot_z = 0,0,0;
	end

	callOpcode(0x067C, {{pedRef,"int"}, {x,"float"}, {y,"float"}, {z,"float"}, {rot_x,"float"}, {rot_y,"float"}, {rot_z,"float"}, {0.0,"float"}, {fif(movecam,1,2),"int"}});
