...
Node = bitser.registerClass(class("NodeShowPickup", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeShowPickup");
...
function Node:play(data, mission)
	local pickup = self:getPinValue(self.id+2,data,mission)[0];
	ldyom.setLastNode(self.id);
	assert(pickup < #mission.list_pickups,"The ID of the pickup exceeds the number of pickups.");
	mission.list_pickups[pickup+1]:updateMissionPickup(mission);
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeApperPickup");
		local name_node = v["class"]["static"]["name"];
		if name == name_node and v["Pins"][v.id+1].value[0] == pickup then
