...
Node = bitser.registerClass(class("NodeHidePickup", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeHidePickup");
...
function Node:play(data, mission)
	...
	mission.list_pickups[pickup+1]:removeMissionPickup();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeDisapperPickup");
