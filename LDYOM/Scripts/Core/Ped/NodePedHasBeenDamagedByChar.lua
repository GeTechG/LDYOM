...
Node = bitser.registerClass(class("NodePedHasBeenDamagedByChar", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("fork")..' '..ldyom.langt("CoreNodePedHasBeenDamagedByChar");
...
function Node:play(data, mission)
	...
	assert(callOpcode(0x056D, {{ped2,"int"}}), "Not found ped 2");
	
	local result = self.Pins[self.id+4].value;
	
	result[0] = callOpcode(0x051A, {{ped,"int"}, {ped2,"int"}});
end
