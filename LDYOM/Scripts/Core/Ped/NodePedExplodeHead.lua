...
Node = bitser.registerClass(class("NodePedExplodeHead", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("fork")..' '..ldyom.langt("CoreNodePedExplodeHead");
...
function Node:play(data, mission)
	...
	assert(callOpcode(0x02CB, {{ped,"int"}}), "Not found ped");
	
	local result = self.Pins[self.id+2].value;
	
	result[0] = callOpcode(0x0321, {{ped,"int"}});
end
