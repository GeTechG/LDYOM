...
Node = bitser.registerClass(class("NodePedTouchingObjectOnFoot", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("fork")..' '..ldyom.langt("CoreNodePedTouchingObjectOnFoot");
...
function Node:play(data, mission)
	...
	local result = self.Pins[self.id+3].value;
	
	result[0] = callOpcode(0x023B, {{ped,"int"}, {object,"int"}});
end
