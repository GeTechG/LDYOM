...
Node = bitser.registerClass(class("NodePedSetIsChrisCriminal", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodePedSetIsChrisCriminal");
...
function Node:draw()
	...
	if not self.Pins[self.id+3].link then
		...
		imgui.ToggleButton(ldyom.langt("value"), self.Pins[self.id+3].value);
	else
...
function Node:play(data, mission)
	...
	local value = self:getPinValue(self.id+3,data,mission)[0];
	ldyom.setLastNode(self.id);
	assert(callOpcode(0x056D, {{ped,"int"}}), "Not found ped");
	
	callOpcode(0x0433, {{ped,"int"}, {value,"bool"}});
end
