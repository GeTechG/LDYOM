...
function Node:initialize(id)
	...
	self.type = 4;
	self.Pins = {
		[self.id+10] = BasePin:new(self.id+10,imgui.imnodes.PinType.void, 0),
		...
		[self.id+13] = BasePin:new(self.id+13,imgui.imnodes.PinType.void, 1),
...
function playNodePreviewAnimPed()
	...
	callOpcode(0x0605,{{PLAYER_PED,"ped"}, {anims[currNodePlayAnim.Pins[currNodePlayAnim.id+3].value[0]+1],"string"}, {ldyom.Anim_name[currNodePlayAnim.Pins[currNodePlayAnim.id+1].value[0]+1],"string"}, {1.0,"float"}, {currNodePlayAnim.Pins[currNodePlayAnim.id+4].value[0],"bool"}, {false,"bool"}, {false,"bool"}, {false,"bool"}, {-1,"int"}});
end
...
function Node:draw()
	...
	imgui.imnodes.BeginStaticAttribute(self.id+11);
	...
	imgui.imnodes.BeginOutputAttribute(self.id+13);
	imgui.Indent(130);
...
function Node:play(data, mission)
	addThreadObj(function(data_pack)
		...
		if not unbreak then
			callOpcode(0x0605,{{ped,"int"}, {anims[anim+1],"string"}, {ldyom.Anim_name[pack+1],"string"}, {1.0,"float"}, {looped,"bool"}, {useAnimMove,"bool"}, {useAnimMove,"bool"}, {stopEndFrame,"bool"}, {fif(_time > 0,_time*1000,-1),"int"}});
        else
			callOpcode(0x088A,{{ped,"int"}, {anims[anim+1],"string"}, {ldyom.Anim_name[pack+1],"string"}, {1.0,"float"}, {looped,"bool"}, {useAnimMove,"bool"}, {useAnimMove,"bool"}, {stopEndFrame,"bool"}, {fif(_time > 0,_time*1000,-1),"int"}, {false,"bool"}, {false,"bool"}});
        end
		if _time > 0 then
            wait(_time * 1000)
        else
		...
		node:callOutputLinks(data, mission, node.id+13);
		
	end, {data,mission,self});
	
	self:callOutputLinks(data, mission, self.id+14);
end
