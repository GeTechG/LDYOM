...
function Node:initialize(id)
	...
	self.Pins = {
		...
		[self.id+7] = BasePin:new(self.id+7,imgui.imnodes.PinType.number, 0, ffi.new("float[1]",x[0])),
		[self.id+8] = BasePin:new(self.id+8,imgui.imnodes.PinType.number, 0, ffi.new("float[1]",y[0])),
		[self.id+9] = BasePin:new(self.id+9,imgui.imnodes.PinType.number, 0, ffi.new("float[1]",z[0])),
		[self.id+10] = BasePin:new(self.id+10,imgui.imnodes.PinType.boolean, 0, ffi.new("bool[1]")),
		[self.id+11] = BasePin:new(self.id+11,imgui.imnodes.PinType.boolean, 1, ffi.new("bool[1]")),
		[self.id+12] = BasePin:new(self.id+12,imgui.imnodes.PinType.boolean, 0, ffi.new("bool[1]")),
	};
end
...
function Node:draw()
	...
	if not self.Pins[self.id+3].link then
		...
		imgui.ComboVecStr("", self.Pins[self.id+3].value, ldyom.langMenu["typeLocate"]);
	end
	imgui.imnodes.EndInputAttribute();
	
	imgui.imnodes.BeginInputAttribute(self.id+12);
	if not self.Pins[self.id+12].link then
		imgui.SetNextItemWidth(200);
		imgui.ToggleButton(ldyom.langt("stopped"), self.Pins[self.id+12].value);
	else
	...
	if imgui.Button(ldyom.langt("playerCoordinates"), imgui.ImVec2:new(200,20)) then
		callOpcode(0x00A0, {{PLAYER_PED,"ped"}, {self.Pins[self.id+7].value,"float*"}, {self.Pins[self.id+8].value,"float*"}, {self.Pins[self.id+9].value,"float*"}});
	end
	imgui.imnodes.EndStaticAttribute();
	...
	if not self.Pins[self.id+10].link then
		imgui.SetNextItemWidth(200);
		imgui.ToggleButton(ldyom.langt("sphere"), self.Pins[self.id+10].value);
	else
...
function Node:play(data, mission)
	...
	local _type = self:getPinValue(self.id+3,data,mission)[0];
	local x1 = self:getPinValue(self.id+4,data,mission)[0];
	local y1 = self:getPinValue(self.id+5,data,mission)[0];
	local z1 = self:getPinValue(self.id+6,data,mission)[0];
	local x2 = self:getPinValue(self.id+7,data,mission)[0];
	local y2 = self:getPinValue(self.id+8,data,mission)[0];
	local z2 = self:getPinValue(self.id+9,data,mission)[0];
	local sphere = self:getPinValue(self.id+10,data,mission)[0];
	local stopped = self:getPinValue(self.id+12,data,mission)[0];
	assert(callOpcode(0x056D, {{ped,"int"}}), "Not found ped");
	ldyom.setLastNode(self.id);
	
	local result = self.Pins[self.id+11].value;
	
	if self.three[0] then
		if stopped then
			if _type == 0 then
				result[0] = callOpcode(0x01A8, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
			elseif _type == 1 then
				result[0] = callOpcode(0x01A9, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
			else
				result[0] = callOpcode(0x01AA, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
			end
		else
			if _type == 0 then
				result[0] = callOpcode(0x00A4, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
			elseif _type == 1 then
				result[0] = callOpcode(0x01A6, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
			else
				result[0] = callOpcode(0x01A7, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {z1,"float"}, {x2,"float"}, {y2,"float"}, {z2,"float"}, {sphere,"bool"}});
			end
		end
	else
		if stopped then
			if _type == 0 then
				result[0] = callOpcode(0x01A3, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
			elseif _type == 1 then
				result[0] = callOpcode(0x01A4, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
			else
				result[0] = callOpcode(0x01A5, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
			end
		else
			if _type == 0 then
				result[0] = callOpcode(0x00A3, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
			elseif _type == 1 then
				result[0] = callOpcode(0x01A1, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
			else
				result[0] = callOpcode(0x01A2, {{ped,"int"}, {x1,"float"}, {y1,"float"}, {x2,"float"}, {y2,"float"}, {sphere,"bool"}});
			end
		end
	end
end
