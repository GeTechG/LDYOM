...
Node = bitser.registerClass(class("NodeHideAudio", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeHideAudio");
...
function Node:play(data, mission)
	...
	mission.list_audios[audio+1]:stop();
	for k,v in pairs(data.nodes) do
		local name = imgui.imnodes.getNodeIcon("event")..' '..ldyom.langt("CoreNodeDisapperAudio");
