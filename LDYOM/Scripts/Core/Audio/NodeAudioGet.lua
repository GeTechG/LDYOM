...
Node = bitser.registerClass(class("NodeAudioGet", BaseNode));
Node.static.mission = true;

Node.static.name = imgui.imnodes.getNodeIcon("func")..' '..ldyom.langt("CoreNodeAudioGet");
...
function Node:draw()
	...
	imgui.Indent(80);
	imgui.Text(ldyom.langt("CoreAudioID"));
...
function Node:play(data, mission)
	local audio = self.Pins[self.id+1].value[0];
	ldyom.setLastNode(self.id);
	self.Pins[self.id+2].value[0] = audio;
	print(tostring(self.Pins[self.id+2].value[0]))
end
