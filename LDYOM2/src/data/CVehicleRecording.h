...
class CPath {
  public:
	...
	size_t Size() const { return m_nBytesSize / sizeof(CVehicleStateEachFrame); }

	auto GetFrames() { return std::span{m_pData, Size()}; }
	...
	// void AddRef() {
	//     CARREC_DEV_LOG("Ref added for path {} (number= {}, size= {}, ptr= {})", GetIndex(), m_nNumber, m_nBytesSize,
	//     LOG_PTR(m_pData)); m_nRefCount++;
	// }

	// void RemoveRef() {
	//     CARREC_DEV_LOG("Ref removed for path {} (number= {}, size= {}, ptr= {})", GetIndex(), m_nNumber,
	//     m_nBytesSize, LOG_PTR(m_pData)); if (!--m_nRefCount) {
	//         Remove();
	//     }
	// }
};
...
class CVehicleRecording {
  public:
	...
	static inline std::array<CPath, TOTAL_RRR_MODEL_IDS>& StreamingArray =
		*(std::array<CPath, TOTAL_RRR_MODEL_IDS>*)0x97D880;
	static inline std::array<CVehicle*, TOTAL_VEHICLE_RECORDS>& pVehicleForPlayback =
		*(std::array<CVehicle*, TOTAL_VEHICLE_RECORDS>*)0x97D840;
	static inline std::array<CVehicleStateEachFrame*, TOTAL_VEHICLE_RECORDS>& pPlaybackBuffer =
		*(std::array<CVehicleStateEachFrame*, TOTAL_VEHICLE_RECORDS>*)0x97D800;
	static inline std::array<INT32, TOTAL_VEHICLE_RECORDS>& PlaybackIndex =
		*(std::array<INT32, TOTAL_VEHICLE_RECORDS>*)0x97D7C0;
	static inline std::array<INT32, TOTAL_VEHICLE_RECORDS>& PlaybackBufferSize =
		*(std::array<INT32, TOTAL_VEHICLE_RECORDS>*)0x97D780;
	static inline std::array<float, TOTAL_VEHICLE_RECORDS>& PlaybackRunningTime =
		*(std::array<float, TOTAL_VEHICLE_RECORDS>*)0x97D740;
	static inline std::array<float, TOTAL_VEHICLE_RECORDS>& PlaybackSpeed =
		*(std::array<float, TOTAL_VEHICLE_RECORDS>*)0x97D700;
	static inline std::array<bool, TOTAL_VEHICLE_RECORDS>& bPlaybackGoingOn =
		*(std::array<bool, TOTAL_VEHICLE_RECORDS>*)0x97D6F0;
	static inline std::array<bool, TOTAL_VEHICLE_RECORDS>& bPlaybackLooped =
		*(std::array<bool, TOTAL_VEHICLE_RECORDS>*)0x97D6E0;
	static inline std::array<bool, TOTAL_VEHICLE_RECORDS>& bPlaybackPaused =
		*(std::array<bool, TOTAL_VEHICLE_RECORDS>*)0x97D6D0;
	static inline std::array<bool, TOTAL_VEHICLE_RECORDS>& bUseCarAI =
		*(std::array<bool, TOTAL_VEHICLE_RECORDS>*)0x97D6C0;
	static inline std::array<UINT32, TOTAL_VEHICLE_RECORDS>& PlayBackStreamingIndex =
		*(std::array<UINT32, TOTAL_VEHICLE_RECORDS>*)0x97D670;
	// DisplayMode
  ...
  public:
	...
	static auto GetActivePlaybackIndices() {
		return std::ranges::views::iota(0, TOTAL_VEHICLE_RECORDS) |
		       std::views::filter([](auto&& i) { return bPlaybackGoingOn[i]; });
	}
