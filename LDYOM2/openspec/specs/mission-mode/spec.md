...
Mission mode is the runtime layer that turns an LDYOM project from a free-roaming script into a GTA mission with a banner, mission flag, and fail/end semantics. The `core.mission_start` and `core.mission_end` objectives bracket the active region. While active, `ProjectPlayer::failCurrentProject()` routes through a banner-driven fail sequence whose action — stop, restart the scene, or transition to another scene — is author-configured per mission, with optional fade-out covering each transition. Outside the active region, fail signals fall through to a hard stop and the system behaves identically to free-roam.
