# Object Scale Specification

## Purpose

Per-object uniform scale for object entities. Scale is stored on the `Object` component and applied through the native `CObject::m_fScale` field, which the GTA engine applies each frame in `CObject::PreRender`. Non-object entity types (actor, vehicle, pickup, particle, firework, checkpoint, train) do not support scale.
