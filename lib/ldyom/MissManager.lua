...
function manager.save(pack)
	...
	print(path..path_pack..name_miss..'.bin')
	f:write(save)
	f:close()
end
...
function manager.save_s(story)
	createDirectory(path_backup)
	local name_story = cyr(replace_symb(ffi.string(vr.temp_var.list_name_storylines[vr.current_storyline[0]+1])))
	local old_save
...
function manager.load_s()
	local name_miss = cyr(replace_symb(ffi.string(vr.temp_var.list_name_storylines[vr.current_storyline[0]+1])))
