...
---@class LDNodeEditorWorldGetRandomCarInSphereNoSaveNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number
---@field radius number
...
local WORLD_GET_RANDOM_CAR_IN_SPHERE_NO_SAVE_NODE_TYPE = "core.world_get_random_car_in_sphere_no_save"

---@type LDNodeEditorNodeType
local worldGetRandomCarInSphereNoSaveNode = {
    typeName = WORLD_GET_RANDOM_CAR_IN_SPHERE_NO_SAVE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorWorldGetRandomCarInSphereNoSaveNode
        local newNode = {
            id = newNodeId,
            nodeType = WORLD_GET_RANDOM_CAR_IN_SPHERE_NO_SAVE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorWorldGetRandomCarInSphereNoSaveNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[6], "model", function ()
            node.model = editor.dataTypes[node.inputs[6].type].drawEditValue(node.model, "##modelEdit", fontScale * 100)
        end);
    ...
    run = function(editor, node, inputValues)
		...
		local model = inputValues[6] or node.model
		local handle = WorldOp.getRandomCarInSphereNoSave(x, y, z, radius, model)
        return {1, handle}
    end
}

return worldGetRandomCarInSphereNoSaveNode
