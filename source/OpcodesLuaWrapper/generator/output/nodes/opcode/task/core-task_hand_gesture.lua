...
---@class LDNodeEditorTaskHandGestureNode : LDNodeEditorNode
---@field handle integer
---@field target integer

local TASK_HAND_GESTURE_NODE_TYPE = "core.task_hand_gesture"

---@type LDNodeEditorNodeType
local taskHandGestureNode = {
    typeName = TASK_HAND_GESTURE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorTaskHandGestureNode
        local newNode = {
            id = newNodeId,
            nodeType = TASK_HAND_GESTURE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorTaskHandGestureNode
    ...
    run = function(editor, node, inputValues)
		...
		TaskOp.handGesture(handle, target)
        return {1}
    end
}

return taskHandGestureNode
