...
---@class LDNodeEditorCharCreateRandomNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number

local CHAR_CREATE_RANDOM_NODE_TYPE = "core.char_create_random"

---@type LDNodeEditorNodeType
local charCreateRandomNode = {
    typeName = CHAR_CREATE_RANDOM_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCharCreateRandomNode
        local newNode = {
            id = newNodeId,
            nodeType = CHAR_CREATE_RANDOM_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCharCreateRandomNode
    ...
    run = function(editor, node, inputValues)
		...
		local handle = CharOp.createRandom(x, y, z)
        return {1, handle}
    end
}

return charCreateRandomNode
