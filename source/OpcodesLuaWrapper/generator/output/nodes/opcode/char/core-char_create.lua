...
---@class LDNodeEditorCharCreateNode : LDNodeEditorNode
...
local CHAR_CREATE_NODE_TYPE = "core.char_create"

---@type LDNodeEditorNodeType
local charCreateNode = {
    typeName = CHAR_CREATE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCharCreateNode
        local newNode = {
            id = newNodeId,
            nodeType = CHAR_CREATE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCharCreateNode
    ...
    run = function(editor, node, inputValues)
		...
		local handle = CharOp.create(pedType, modelId, x, y, z)
        return {1, handle}
    end
}

return charCreateNode
