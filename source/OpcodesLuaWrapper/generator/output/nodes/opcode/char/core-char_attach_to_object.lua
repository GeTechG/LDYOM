...
---@class LDNodeEditorCharAttachToObjectNode : LDNodeEditorNode
---@field self_ integer
...
local CHAR_ATTACH_TO_OBJECT_NODE_TYPE = "core.char_attach_to_object"

---@type LDNodeEditorNodeType
local charAttachToObjectNode = {
    typeName = CHAR_ATTACH_TO_OBJECT_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorCharAttachToObjectNode
        local newNode = {
            id = newNodeId,
            nodeType = CHAR_ATTACH_TO_OBJECT_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorCharAttachToObjectNode
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[7], "orientation", function ()
            node.orientation = editor.dataTypes[node.inputs[7].type].drawEditValue(node.orientation, "##orientationEdit", fontScale * 100)
        end);
