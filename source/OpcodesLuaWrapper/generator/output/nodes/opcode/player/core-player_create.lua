...
---@class LDNodeEditorPlayerCreateNode : LDNodeEditorNode
---@field playerIndex integer
---@field x number
---@field y number
---@field z number

local PLAYER_CREATE_NODE_TYPE = "core.player_create"

---@type LDNodeEditorNodeType
local playerCreateNode = {
    typeName = PLAYER_CREATE_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorPlayerCreateNode
        local newNode = {
            id = newNodeId,
            nodeType = PLAYER_CREATE_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[2], "playerIndex", function ()
            node.playerIndex = editor.dataTypes[node.inputs[2].type].drawEditValue(node.playerIndex, "##playerIndexEdit", fontScale * 100)
        end);
