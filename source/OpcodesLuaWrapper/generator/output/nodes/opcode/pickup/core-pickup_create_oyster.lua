...
---@class LDNodeEditorPickupCreateOysterNode : LDNodeEditorNode
---@field x number
---@field y number
---@field z number

local PICKUP_CREATE_OYSTER_NODE_TYPE = "core.pickup_create_oyster"

---@type LDNodeEditorNodeType
local pickupCreateOysterNode = {
    typeName = PICKUP_CREATE_OYSTER_NODE_TYPE,
    ...
    new = function(ctx, newNodeId, getPinId)
        ---@type LDNodeEditorPickupCreateOysterNode
        local newNode = {
            id = newNodeId,
            nodeType = PICKUP_CREATE_OYSTER_NODE_TYPE,
            inputs = {
                {
                    id = getPinId(),
                    node = newNodeId,
    ...
    ---@param ctx LDNodeEditorContext
    ---@param node LDNodeEditorPickupCreateOysterNode
    ...
    run = function(editor, node, inputValues)
		...
		local handle = PickupOp.createOyster(x, y, z)
        return {1, handle}
    end
}

return pickupCreateOysterNode
