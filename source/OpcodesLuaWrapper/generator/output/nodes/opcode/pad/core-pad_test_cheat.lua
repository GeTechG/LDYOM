...
local padTestCheatNode = {
    ...
    draw = function(editor, ctx, node, builder)
		...
		LDNodeEditor.defaultInput(editor, ctx, builder, node.inputs[2], "input", function ()
            node.input = editor.dataTypes[node.inputs[2].type].drawEditValue(node.input, "##inputEdit", fontScale * 100)
        end);
    ...
    run = function(editor, node, inputValues)
        local input = inputValues[2] or node.input
