...
void bindHeli(sol::state &state) {
  ...
  table.set_function("attackPlayer", [](const int &_self, const int &_handle, const float &_radius) {
    Command<0x0724>(_self, _handle, _radius);
  });
  table.set_function("followEntity", [](const int &_self, const int &_char, const int &_vehicle, const float &_radius) {
    Command<0x0726>(_self, _char, _vehicle, _radius);
  });
  table.set_function("chaseEntity", [](const int &_self, const int &_char, const int &_vehicle, const float &_radius) {
    Command<0x0727>(_self, _char, _vehicle, _radius);
  });
  table.set_function("landAtCoords",
                     [](const int &_self, const float &_x, const float &_y, const float &_z, const float &_minAltitude,
                        const float &_maxAltitude) { Command<0x0743>(_self, _x, _y, _z, _minAltitude, _maxAltitude); });
  table.set_function("keepEntityInView", [](const int &_self, const int &_char, const int &_vehicle,
  ...
  table.set_function("grabEntityOnWinch", [](const int &_self) {
    ...
    int vehicle_;
    int object_;
    Command<0x078B>(_self, &char_, &vehicle_, &object_);
    return std::make_tuple(char_, vehicle_, object_);
  });
  table.set_function("activateSpeedCheat", [](const int &_self, const int &_power) { Command<0x07BB>(_self, _power); });
