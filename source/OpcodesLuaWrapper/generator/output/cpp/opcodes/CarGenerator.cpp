...
void bindCarGenerator(sol::state &state) {
  ...
  table.set_function("create", [](const float &_x, const float &_y, const float &_z, const float &_heading,
                                  ...
                                  const bool &_forceSpawn, const int &_alarmChance, const int &_doorLockChance,
  ...
  table.set_function("setHasBeenOwned", [](const int &_self, const bool &_state) { Command<0x0A17>(_self, _state); });
