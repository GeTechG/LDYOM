...
void bindCamera(sol::state &state) {
  ...
  table.set_function("setTwoPlayerMode", [](const bool &_state) { Command<0x06E0>(_state); });
  table.set_function("getFov", []() {
    float fov_;
    Command<0x0801>(&fov_);
    return std::make_tuple(fov_);
  });
  table.set_function("setFirstPersonInCarMode", [](const bool &_state) { Command<0x0822>(_state); });
  ...
  table.set_function("setVectorTrack",
                     [](const float &_fromX, const float &_fromY, const float &_fromZ, const float &_toX,
                        const float &_toY, const float &_toZ, const int &_time, const bool &_ease) {
                       Command<0x0920>(_fromX, _fromY, _fromZ, _toX, _toY, _toZ, _time, _ease);
                     });
  table.set_function("setLerpFov", [](const float &_from, const float &_to, const int &_time, const bool &_ease) {
    Command<0x0922>(_from, _to, _time, _ease);
  });
  table.set_function("setDarknessEffect",
                     [](const bool &_enable, const int &_pitchBlack) { Command<0x0924>(_enable, _pitchBlack); });
  table.set_function("resetNewScriptables", []() { Command<0x0925>(); });
  table.set_function("persistTrack", [](const bool &_state) { Command<0x092F>(_state); });
  table.set_function("persistPos", [](const bool &_state) { Command<0x0930>(_state); });
  table.set_function("persistFov", [](const bool &_state) { Command<0x0931>(_state); });
  ...
  table.set_function("setVectorMove",
                     [](const float &_fromX, const float &_fromY, const float &_fromZ, const float &_toX,
                        const float &_toY, const float &_toZ, const int &_time, const bool &_ease) {
                       Command<0x0936>(_fromX, _fromY, _fromZ, _toX, _toY, _toZ, _time, _ease);
                     });
  table.set_function("setCinema", [](const bool &_state) { Command<0x093D>(_state); });
  ...
  table.set_function("allowFixedCollision", [](const bool &_state) { Command<0x09EC>(_state); });
  ...
  table.set_function("takePhoto", [](const bool &__p1) { Command<0x0A1E>(__p1); });
  table.set_function("setPositionUnfixed",
                     [](const float &_xOffset, const float &_yOffset) { Command<0x0A25>(_xOffset, _yOffset); });
  table.set_function("setPhotoEffect", [](const bool &_state) { Command<0x0A2F>(_state); });
  ...
  // CLEO+
  table.set_function("setCameraControl", [](const bool &_state) { Command<0x0E60>(_state); });
