...
void EnterVehicleActorObjective::draw(Localization& local) {
	...
	ImGui::SliderInt(local.get("general.place").c_str(), &this->place_, 0, 3, local.getArray("general.places_in_car").at(this->place_).c_str());
...
ktwait EnterVehicleActorObjective::execute(Scene* scene, Actor* actor, Result& result, ktcoro_tasklist& tasklist) {
	...
	if (this->teleport_) {
		if (this->place_ == 0) {
			Command<Commands::TASK_WARP_CHAR_INTO_CAR_AS_DRIVER>(actor->getProjectPed().value(), vehicle->getProjectVehicle().value());
		}
		else {
			Command<Commands::TASK_WARP_CHAR_INTO_CAR_AS_PASSENGER>(actor->getProjectPed().value(), vehicle->getProjectVehicle().value(), this->place_ - 1);
		}
	} else {
		if (this->place_ == 0) {
			Command<Commands::TASK_ENTER_CAR_AS_DRIVER>(actor->getProjectPed().value(), vehicle->getProjectVehicle().value(), -1);
		}
		else {
			Command<Commands::TASK_ENTER_CAR_AS_PASSENGER>(actor->getProjectPed().value(), vehicle->getProjectVehicle().value(), -1, this->place_ - 1);
		}
	}

	if (this->waitExecute_) {
		while(!Command<Commands::IS_CHAR_IN_CAR>(actor->getProjectPed().value(), vehicle->getProjectVehicle().value())) {
			co_await 1;
		}
	}
}
