...
void TeleportToVehiclePlayerObjective::draw(Localization& local) {
	...
	ImGui::SliderInt(local.get("general.place").c_str(), &this->place_, 0, 3,local.getArray("general.places_in_car")[this->place_].c_str());
}
...
ktwait TeleportToVehiclePlayerObjective::execute(Scene* scene, Result& result, ktcoro_tasklist& tasklist) {
	...
	if (this->place_ == 0)
		Command<Commands::TASK_WARP_CHAR_INTO_CAR_AS_DRIVER>(static_cast<CPed*>(FindPlayerPed()), vehicle->getProjectVehicle().value());
	else
		Command<Commands::TASK_WARP_CHAR_INTO_CAR_AS_PASSENGER>(static_cast<CPed*>(FindPlayerPed()), vehicle->getProjectVehicle().value(), this->place_ - 1);
