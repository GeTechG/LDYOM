...
void FollowPathActorObjective::draw(Localization& local) {
	...
	ImGui::SliderInt(local.get("follow_path_actor_objective.move_type").c_str(), &this->moveType_, 0, 2, local.getArray("follow_path_actor_objective.move_states").at(this->moveType_).c_str());
...
ktwait FollowPathActorObjective::execute(Scene* scene, Actor* actor, Result& result, ktcoro_tasklist& tasklist) {
	...
	tasklist.add_task([](FollowPathActorObjective* _this, Actor* actor) -> ktwait {
		...
		while (execute) {
			...
			Command<Commands::TASK_GO_STRAIGHT_TO_COORD>(actor->getProjectPed().value(), x, y, z, moveState[_this->getMoveType()], -1);
			...
			while (DistanceBetweenPoints(CVector(x, y, z), actor->getProjectPed().value()->GetPosition()) > reachDistance && timeCondition) {
