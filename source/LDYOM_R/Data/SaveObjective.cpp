...
#include "../ImGui/WindowsRenderService.h"
...
void SaveObjective::saveGame(bool confirm, int nodeSave, Scene* scene, ktcoro_tasklist& tasklist, std::function<void()> callback) {
	tasklist.add_task([](const bool confirm, const int nodeSave, Scene* scene, std::function<void()> callback) -> ktwait {
		auto saveGame = [&]() {
			...
			for (auto pairLua : luaState["global_data"]["signals"]["saveGame"].get_or_create<sol::table>()) {
				if (auto resultLua = pairLua.second.as<sol::function>()(scene->getId(), luaData); !resultLua.valid()) {
			...
			SaveService::getInstance().addSave(ProjectsService::getInstance().getCurrentProject().getProjectInfo()->name,
				SaveData {
				scene->getId(),
					luaDataStr,
		...
		if (confirm) {
			...
			const auto saveConfirmPopup = Windows::WindowsRenderService::getInstance().getWindow<Windows::SaveConfirmPopup>();
