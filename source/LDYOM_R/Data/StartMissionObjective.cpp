...
void StartMissionObjective::draw(Localization &local) {
		...
		if (ImGui::TreeNode(fmt::format("{} {}", local.get("actor.group"), i + 1).c_str())) {
			for (int j = 0; j < 9; j++) {
				if (this->groupRelations[i][j] != -1) {
					...
					ImGui::SliderInt(groupName.c_str(), &this->groupRelations[i][j], 0, 4,
					                 local.getArray("scene_settings.relationship_types").at(this->groupRelations[i][j]).
					                       c_str());
				}
			}
			ImGui::TreePop();
	...
	ImGui::SliderInt(fmt::format("{} {}", ICON_FA_STAR, local.get("general.min_wanted")).c_str(), &this->wantedMin, 0,
	                 6);
	ImGui::SliderInt(fmt::format("{} {}", ICON_FA_STAR, local.get("general.max_wanted")).c_str(), &this->wantedMax, 0,
	                 6);
...
ktwait StartMissionObjective::execute(Scene *scene, Result &result, ktcoro_tasklist &tasklist) {
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 9; j++) {
			if (this->groupRelations[i][j] != -1) {
				if (j == 0) {
					Command<Commands::SET_RELATIONSHIP>(this->groupRelations[i][j], 24 + i, 0);
					Command<Commands::SET_RELATIONSHIP>(this->groupRelations[i][j], 24 + i, 23);
					Command<Commands::SET_RELATIONSHIP>(this->groupRelations[i][j], 23, 24 + i);
				} else {
					if (i != j - 1) {
						Command<Commands::SET_RELATIONSHIP>(this->groupRelations[i][j], 24 + i, 24 + j - 1);
					}
				}
			}
		}
	...
	if (this->showNameMission_) {
		...
		CMessages::AddBigMessage(const_cast<char*>(cp1251Text.c_str()),
		                         ...
		                         ModelsService::getInstance().getTextStyles().at(this->styleTypeName_));
	}
