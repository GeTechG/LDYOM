...
void EndMissionObjective::draw(Localization& local) {
	...
	ImGui::SliderInt(local.get("general.text_style").c_str(), &this->styleTypeEndText_, 0, 6, local.getArray("general.text_styles").at(this->styleTypeEndText_).c_str());
...
ktwait EndMissionObjective::execute(Scene* scene, Result& result, ktcoro_tasklist& tasklist) {
	...
	if (this->showEndTextMission_) {
		...
		CMessages::AddBigMessage(const_cast<char*>(cp1251Text.c_str()), static_cast<unsigned>(this->endTextMissionTime_ * 1000.f), ModelsService::getInstance().getTextStyles().at(this->styleTypeEndText_));
	}
