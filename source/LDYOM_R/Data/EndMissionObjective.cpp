...
bool& EndMissionObjective::isShowEndTextMission() { return showEndTextMission_; }
std::string& EndMissionObjective::getEndTextMission() { return endTextMission_; }
int& EndMissionObjective::getStyleTypeEndText() { return styleTypeEndText_; }
float& EndMissionObjective::getEndTextMissionTime() { return endTextMissionTime_; }
int& EndMissionObjective::getEndMissionTune() { return endMissionTune_; }
...
void EndMissionObjective::draw(Localization &local, std::vector<std::string> &listOverlay) {
	...
	ImGui::InputText(local.get("general.text").c_str(), &this->endTextMission_);
...
ktwait EndMissionObjective::execute(Scene *scene, Result &result, ktcoro_tasklist &tasklist) {
	...
	if (this->showEndTextMission_) {
		...
		cp1251Text = utf8ToCp1251(this->endTextMission_);
