...
namespace ImGui {
	namespace StyleLoader {
		...
		template<typename T>
		T get_number(const toml::node* node) {
			if (node->type() == toml::node_type::floating_point)
				return static_cast<T>(node->ref<double>());
			return static_cast<T>(node->ref<int64_t>());
		}
		...
		static inline bool _handle_node(std::string const& fqn,
			...
			if (meta_it != g_nodes_meta.end()) {
				...
				case NodeValueType::Vector2Float: {
					if (auto value = &table->get(VAR_VALUE_NAME)->ref<toml::array>()) {
						...
						ImVec2 const new_value = { (float)get_number<double>(values.get(0)), (float)get_number<double>(values.get(1)) };
				...
				case NodeValueType::Vector4Float: {
					if (auto value = &table->get(VAR_VALUE_NAME)->ref<toml::array>()) {
						...
						ImVec4 const new_value = { (float)get_number<double>(values.get(0)), (float)get_number<double>(values.get(1)), (float)get_number<double>(values.get(2)), (float)get_number<double>(values.get(3)) };
						if (node_meta.type == NodeType::Variable) {
		...
		static bool LoadFile(std::string const& path) {
			try {
				auto const config = toml::parse_file(path);
			...
			catch (toml::ex::parse_error const& e) {
				LOGTEXT("toml: %s", e.what());
				return false;
			}
