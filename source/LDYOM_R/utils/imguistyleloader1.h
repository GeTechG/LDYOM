...
static inline bool _handle_node(std::string const &fqn,
                                toml::impl::table_proxy_pair<false>::value_type const &current_node) {
    ...
    if (meta_it != g_nodes_meta.end()) {
        ...
        switch (node_meta.value_type) {
            case NodeValueType::Float: {
                if (auto value = table->get(VAR_VALUE_NAME)->as_floating_point()->value_or(.0)) {
            ...
            case NodeValueType::Vector2Float: {
                if (auto value = table->get(VAR_VALUE_NAME)->as_array()) {
                    ...
                    ImVec2 const new_value = {values[0].as_floating_point()->value_or(.0f), values[1].as_floating_point()->value_or(.0f)};
            ...
            case NodeValueType::Vector4Float: {
                if (auto value = table->get(VAR_VALUE_NAME)->as_array()) {
                    ...
                    ImVec4 const new_value = {values[0].as_floating_point()->value_or(.0f), values[1].as_floating_point()->value_or(.0f), values[2].as_floating_point()->value_or(.0f), values[3].as_floating_point()->value_or(.0f) };
                    if (node_meta.type == NodeType::Variable) {
