...
public:
	...
	public:
		...
		void setColor(CRGBA color) {
			VehicleRenderer::get()->setMaterialColor(_vehicle, _material, _geometry,
		...
		static VehicleMaterial fromMemory(uintptr_t vehicle, uintptr_t mat, uintptr_t geometry) {
			return VehicleMaterial{
				reinterpret_cast<CVehicle*>(vehicle), reinterpret_cast<RpGeometry*>(geometry),
				reinterpret_cast<RpMaterial*>(mat)
			};
		}
	}; // VehicleMaterial
	...
	public:
		...
		void setAlpha(unsigned char alpha) {
			RwFrameForAllObjects(_frame, reinterpret_cast<RwObjectCallBack>(CVehicle::SetComponentAtomicAlpha),
	...
	static vector<VehicleComponent> get_all_vehicle_components(CVehicle *vehicle) {
		return getVehicleComponents<decltype(&storeAllVehicleComponents), &storeAllVehicleComponents>(vehicle);
	}

	static void extractComObjMatVehicle(CVehicle *vehicle,
	                                    std::function<void(int, VehicleComponent, VehicleAtomic, VehicleMaterial)>
	                                    callback) {
		int i = 0;
