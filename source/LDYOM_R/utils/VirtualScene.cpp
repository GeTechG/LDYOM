...
void add_camera_to_gta_scene(RwCamera* camera) {
	if (Scene.m_pRpWorld)
		RpWorldAddCamera(Scene.m_pRpWorld, camera);
}

void add_light_to_gta_scene(RpLight* light) {
	if (Scene.m_pRpWorld)
		RpWorldAddLight(Scene.m_pRpWorld, light);
}

void remove_light_from_gta_scene(RpLight* light) {
	if (Scene.m_pRpWorld)
		RpWorldRemoveLight(Scene.m_pRpWorld, light);
}
...
bool VirtualScene::initialize() {
	...
	constexpr RwV2d viewWin{0.5f, 0.5f};
	RwCameraSetViewWindow(camera_, &viewWin);
...
RwTexture* VirtualScene::renderVehicle(int model, const RwRGBA bgr, const RwV3d& pos, const RwV3d& rotate, const int primary,
                                       const int secondary) {
	if (model == 570)
		model = 538;
	else if (model == 569)
