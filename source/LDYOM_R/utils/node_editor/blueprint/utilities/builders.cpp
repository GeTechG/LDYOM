...
void util::BlueprintNodeBuilder::Begin(NodeId id) {
	HasHeader = false;
...
void util::BlueprintNodeBuilder::End() {
	...
	if (ImGui::IsItemVisible()) {
		...
		const auto halfBorderWidth = GetStyle().NodeBorderWidth * 0.5f;
...
void util::BlueprintNodeBuilder::Input(PinId id) {
...
void util::BlueprintNodeBuilder::Output(PinId id) {
	if (CurrentStage == Stage::Begin)
		SetStage(Stage::Content);
...
bool util::BlueprintNodeBuilder::SetStage(Stage stage) {
	...
	switch (stage) {
		...
		case Stage::Input:
			...
			PushStyleVar(StyleVar_PivotAlignment, ImVec2(0, 0.5f));
			PushStyleVar(StyleVar_PivotSize, ImVec2(0, 0));
		...
		case Stage::Output:
			...
			PushStyleVar(StyleVar_PivotAlignment, ImVec2(1.0f, 0.5f));
			PushStyleVar(StyleVar_PivotSize, ImVec2(0, 0));
...
void util::BlueprintNodeBuilder::Pin(PinId id, PinKind kind) {
	BeginPin(id, kind);
}
