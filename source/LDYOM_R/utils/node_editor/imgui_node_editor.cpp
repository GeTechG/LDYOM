...
static void ImDrawList_PathBezierOffset(ImDrawList *drawList, float offset, const ImVec2 &p0, const ImVec2 &p1,
...
static void ImDrawList_AddBezierWithArrows(ImDrawList *drawList, const ImCubicBezierPoints &curve, float thickness,
	...
	if (fill) {
		...
		if (startArrowSize > 0.0f) {
			const auto start_dir = ImNormalized(startDirHint
				                                    ? *startDirHint
				                                    : ImCubicBezierTangent(
					                                    curve.P0, curve.P1, curve.P2, curve.P3, 0.0f));
		...
		if (endArrowSize > 0.0f) {
			const auto end_dir = ImNormalized(endDirHint
				                                  ? -*endDirHint
...
void ed::Node::DrawBorder(ImDrawList *drawList, ImU32 color, float thickness, float offset) {
	if (thickness > 0.0f) {
		const auto extraOffset = ImVec2(offset, offset);
...
void ed::EditorContext::Begin(const char *id, const ImVec2 &size) {
	...
	if (!ImRect_IsEmpty(previousVisibleRect) && previousSize != canvasSize) {
		...
		auto currentAspectRatio = currentVisibleRect.GetHeight()
			                          ? (currentVisibleRect.GetWidth() / currentVisibleRect.GetHeight())
			                          : 0.0f;
		auto width = previousVisibleRect.GetWidth();
		auto height = previousVisibleRect.GetHeight();

		if (m_Config.CanvasSizeMode == CanvasSizeMode::FitVerticalView) {
			height = previousVisibleRect.GetHeight();
			width = height * currentAspectRatio;
		} else if (m_Config.CanvasSizeMode == CanvasSizeMode::FitHorizontalView) {
			width = previousVisibleRect.GetWidth();
			height = width / currentAspectRatio;
		} else if (m_Config.CanvasSizeMode == CanvasSizeMode::CenterOnly) {
		...
		m_NavigateAction.NavigateTo(previousVisibleRect, NavigateAction::ZoomMode::Exact, 0.0f);
	}
...
void ed::EditorContext::End() {
	...
	// Highlight selected objects
	{
		...
		for (auto &link : m_Links) {
			...
			auto isLinkHighlighted = isLinkHighlightedForPin(*link->m_StartPin) || isLinkHighlightedForPin(
				*link->m_EndPin);
	...
	if (nullptr == m_CurrentAction) {
		...
		auto accept = [&possibleAction, &control](EditorAction &action) {
			...
			if (result == EditorAction::True)
				return true;
			if (/*!possibleAction &&*/ result == EditorAction::Possible)
	...
	if (sortGroups || ((m_Settings.m_DirtyReason & (SaveReasonFlags::Position | SaveReasonFlags::Size)) !=
		...
		std::sort(m_Nodes.begin(), groupsItEnd, [this](Node *lhs, Node *rhs) {
			const auto &lhsSize = lhs == m_SizeAction.m_SizedNode
				                      ? m_SizeAction.GetStartGroupBounds().GetSize()
				                      : lhs->m_GroupBounds.GetSize();
			const auto &rhsSize = rhs == m_SizeAction.m_SizedNode
				                      ? m_SizeAction.GetStartGroupBounds().GetSize()
				                      : rhs->m_GroupBounds.GetSize();
...
# if 1
	...
	// node drawing order.
	{
		...
		ImDrawList_ChannelsGrow(
			m_DrawList, m_DrawList->_Splitter._Count + c_ChannelsPerNode * liveNodeCount + c_LinkChannelCount);
...
# endif
	...
	// Draw border
	{
		...
		m_DrawList->AddRect(m_Canvas.Rect().Min + ImVec2(1, 1), m_Canvas.Rect().Max - ImVec2(1, 1),
		                    ImColor(borderShadoColor));
...
void ed::EditorContext::SetNodePosition(NodeId nodeId, const ImVec2 &position) {
	...
	if (node->m_Bounds.Min != position) {
		...
		node->m_Bounds.Floor();
		MakeDirty(SaveReasonFlags::Position, node);
	}
}
...
int ed::EditorContext::CountLiveNodes() const {
	return std::count_if(m_Nodes.begin(), m_Nodes.end(), [](const Node *node) { return node->m_IsLive; });
}

int ed::EditorContext::CountLivePins() const {
	return std::count_if(m_Pins.begin(), m_Pins.end(), [](const Pin *pin) { return pin->m_IsLive; });
}

int ed::EditorContext::CountLiveLinks() const {
	return std::count_if(m_Links.begin(), m_Links.end(), [](const Link *link) { return link->m_IsLive; });
}
...
void ed::EditorContext::UpdateAnimations() {
	...
	for (auto animation : m_LastLiveAnimations) {
		const bool isLive = (std::find(m_LiveAnimations.begin(), m_LiveAnimations.end(), animation) != m_LiveAnimations.
			end());
...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen) {
	...
	static auto invisibleButtonEx = [](const char *str_id, const ImVec2 &size_arg, ImGuiButtonFlags extraFlags) -> int {
		...
		auto buttonIndex = GetCurrentContext()->ActiveIdMouseButton;
	...
	auto checkInteractionsInArea = [this, &emitInteractiveArea, &hotObject, &activeObject, &clickedObject, &
			doubleClickedObject](ObjectId id, const ImRect &rect, Object *object) {
	...
	auto isMouseDoubleClickOverBackground = [doubleClickedObject, backgroundExtraFlags]() -> int {
		...
		if ((backgroundExtraFlags & ImGuiButtonFlags_MouseButtonLeft) && ImGui::IsMouseDoubleClicked(
			ImGuiMouseButton_Left))
			return ImGuiButtonFlags_MouseButtonLeft;
		if ((backgroundExtraFlags & ImGuiButtonFlags_MouseButtonRight) && ImGui::IsMouseDoubleClicked(
			ImGuiButtonFlags_MouseButtonRight))
			return ImGuiButtonFlags_MouseButtonRight;
		if ((backgroundExtraFlags & ImGuiButtonFlags_MouseButtonMiddle) && ImGui::IsMouseDoubleClicked(
...
void ed::EditorContext::ShowMetrics(const Control &control) {
	...
	auto getObjectName = [](Object *object) {
		if (!object) return "";
		if (object->AsNode()) return "Node";
		if (object->AsPin()) return "Pin";
		if (object->AsLink()) return "Link";
		return "";
	};
	...
	ImGui::Text("Mouse: { x=%.0f y=%.0f } global: { x=%g y=%g }", localMousePos.x, localMousePos.y, globalMousePos.x,
	            globalMousePos.y);
...
void ed::FlowAnimation::Flow(Link *link, float markerDistance, float speed, float duration) {
	Stop();
...
ImVec2 ed::FlowAnimation::SamplePath(float distance) const {
	...
	auto endPointIt = std::find_if(m_Path.begin(), m_Path.end(), [distance](const CurvePoint &p) {
...
void ed::FlowAnimationController::Flow(Link *link, FlowDirection direction) {
	...
	animation->Flow(link, editorStyle.FlowMarkerDistance, editorStyle.FlowSpeed * speedDirection,
	                editorStyle.FlowDuration);
}
...
ed::FlowAnimation* ed::FlowAnimationController::GetOrCreate(Link *link) {
	...
	{
		auto animationIt = std::find_if(m_Animations.begin(), m_Animations.end(), [link](FlowAnimation *animation) {
			return animation->m_Link == link;
		});
...
ed::NavigateAction::NavigateAction(EditorContext *editor, ImGuiEx::Canvas &canvas):
	...
	m_ZoomLevels(editor->GetConfig().CustomZoomLevels.Size > 0
		             ? editor->GetConfig().CustomZoomLevels.Data
		             : s_DefaultZoomLevels),
	m_ZoomLevelCount(editor->GetConfig().CustomZoomLevels.Size > 0
		                 ? editor->GetConfig().CustomZoomLevels.Size
		                 : s_DefaultZoomLevelCount) {}
...
ed::EditorAction::AcceptResult ed::NavigateAction::Accept(const Control &control) {
	...
	if (Editor->CanAcceptUserInput() /*&& !ImGui::IsAnyItemActive()*/ && ImGui::IsMouseDragging(
		Editor->GetConfig().NavigateButtonIndex, 0.0f)) {
		m_IsActive = true;
	...
	if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(GetKeyIndexForF()) && Editor->AreShortcutsEnabled()) {
		const auto zoomMode = io.KeyShift ? ZoomMode::WithMargin : ZoomMode::None;
		...
		if (!Editor->GetSelectedObjects().empty()) {
			if (m_Reason != NavigationReason::Selection || m_LastSelectionId != Editor->GetSelectionId() || (zoomMode !=
				ZoomMode::None)) {
			...
			if (m_Reason != NavigationReason::Object || m_LastObject != hotObject || (zoomMode != ZoomMode::None)) {
		...
		if (navigateToContent)
			NavigateTo(Editor->GetContentBounds(), ZoomMode::WithMargin, -1.0f, NavigationReason::Content);
	}
...
bool ed::NavigateAction::HandleZoom(const Control &control) {
	...
	if (m_Scroll != savedScroll || m_Zoom != savedZoom || m_VisibleRect.Min != visibleRect.Min || m_VisibleRect.Max !=
		visibleRect.Max) {
...
float ed::NavigateAction::GetNextZoom(float steps) {
	...
	auto fixedSteps = static_cast<int>(steps);
...
float ed::NavigateAction::MatchSmoothZoom(float steps) {
	...
	if (newZoom > m_ZoomLevels[m_ZoomLevelCount - 1])
...
ed::EditorAction::AcceptResult ed::SizeAction::Accept(const Control &control) {
	...
	if (control.ActiveNode && IsGroup(control.ActiveNode) && ImGui::IsMouseDragging(
		Editor->GetConfig().DragButtonIndex, 1)) {
...
bool ed::SizeAction::Process(const Control &control) {
	if (m_Clean) {
		...
		if (m_SizedNode->m_Bounds.GetSize() != m_StartBounds.GetSize() || m_SizedNode->m_GroupBounds.GetSize() !=
			m_StartGroupBounds.GetSize())
	...
	if (control.ActiveNode == m_SizedNode) {
		const auto dragOffset = (control.ActiveNode == m_SizedNode)
		...
		if ((m_Pivot & NodeRegion::Top) == NodeRegion::Top)
			newBounds.Min.y = ImMin(newBounds.Max.y - minimumSize.y,
			                        Editor->AlignPointToGrid(newBounds.Min.y + dragOffset.y));
		if ((m_Pivot & NodeRegion::Bottom) == NodeRegion::Bottom)
			newBounds.Max.y = ImMax(newBounds.Min.y + minimumSize.y,
			                        Editor->AlignPointToGrid(newBounds.Max.y + dragOffset.y));
		if ((m_Pivot & NodeRegion::Left) == NodeRegion::Left)
			newBounds.Min.x = ImMin(newBounds.Max.x - minimumSize.x,
			                        Editor->AlignPointToGrid(newBounds.Min.x + dragOffset.x));
		if ((m_Pivot & NodeRegion::Right) == NodeRegion::Right)
			newBounds.Max.x = ImMax(newBounds.Min.x + minimumSize.x,
			                        Editor->AlignPointToGrid(newBounds.Max.x + dragOffset.x));
...
void ed::SizeAction::ShowMetrics() {
	...
	if (m_SizedNode && m_IsActive) {
		ImGui::Text("    Bounds: { x=%g y=%g w=%g h=%g }", m_SizedNode->m_Bounds.Min.x, m_SizedNode->m_Bounds.Min.y,
		            m_SizedNode->m_Bounds.GetWidth(), m_SizedNode->m_Bounds.GetHeight());
		ImGui::Text("    Group Bounds: { x=%g y=%g w=%g h=%g }", m_SizedNode->m_GroupBounds.Min.x,
		            m_SizedNode->m_GroupBounds.Min.y, m_SizedNode->m_GroupBounds.GetWidth(),
		            m_SizedNode->m_GroupBounds.GetHeight());
		ImGui::Text("    Start Bounds: { x=%g y=%g w=%g h=%g }", m_StartBounds.Min.x, m_StartBounds.Min.y,
		            m_StartBounds.GetWidth(), m_StartBounds.GetHeight());
		ImGui::Text("    Start Group Bounds: { x=%g y=%g w=%g h=%g }", m_StartGroupBounds.Min.x,
		            m_StartGroupBounds.Min.y, m_StartGroupBounds.GetWidth(), m_StartGroupBounds.GetHeight());
...
ed::EditorAction::AcceptResult ed::DragAction::Accept(const Control &control) {
	...
	if (Editor->CanAcceptUserInput() && control.ActiveObject && ImGui::IsMouseDragging(
		Editor->GetConfig().DragButtonIndex, 1)) {
		...
		m_IsActive = true;
	} else if (control.HotNode && IsGroup(control.HotNode) && control.HotNode->GetRegion(ImGui::GetMousePos()) ==
		NodeRegion::Header) {
		return Possible;
	}
...
bool ed::DragAction::Process(const Control &control) {
	...
	if (control.ActiveObject == m_DraggedObject) {
		...
		auto alignedOffset = Editor->AlignPointToGrid(draggedOrigin + dragOffset + alignPivot) - draggedOrigin -
			alignPivot;
...
void ed::DragAction::ShowMetrics() {
	...
	ImGui::Text("%s:", GetName());
	ImGui::Text("    Active: %s", m_IsActive ? "yes" : "no");
	ImGui::Text("    Node: %s (%p)", getObjectName(m_DraggedObject),
	            m_DraggedObject ? m_DraggedObject->ID().AsPointer() : nullptr);
}
...
ed::EditorAction::AcceptResult ed::SelectAction::Accept(const Control &control) {
	...
	if (Editor->CanAcceptUserInput() && control.BackgroundHot && ImGui::IsMouseDragging(
		Editor->GetConfig().SelectButtonIndex, 1)) {
		m_IsActive = true;
		...
		Object *clickedObject = control.ClickedNode
			                        ? static_cast<Object*>(control.ClickedNode)
			                        : static_cast<Object*>(control.ClickedLink);
...
bool ed::SelectAction::Process(const Control &control) {
	...
	if (ImGui::IsMouseDragging(Editor->GetConfig().SelectButtonIndex, 0)) {
		...
		m_CandidateObjects.insert(m_CandidateObjects.end(), m_SelectedObjectsAtStart.begin(),
		                          m_SelectedObjectsAtStart.end());
		std::sort(m_CandidateObjects.begin(), m_CandidateObjects.end());
		m_CandidateObjects.erase(std::unique(m_CandidateObjects.begin(), m_CandidateObjects.end()),
		                         m_CandidateObjects.end());
	} else {
		m_IsActive = false;
...
void ed::SelectAction::Draw(ImDrawList *drawList) {
	...
	const auto outlineColor = Editor->GetColor(
		m_SelectLinkMode ? StyleColor_LinkSelRectBorder : StyleColor_NodeSelRectBorder, alpha);
...
ed::EditorAction::AcceptResult ed::ContextMenuAction::Accept(const Control &control) {
	...
	if (isPressed || isReleased || isDragging) {
		...
		if (isReleased && m_CandidateMenu == candidateMenu && m_ContextId == contextId) {
...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control &control) {
	...
	if (candidateAction != None) {
		if (candidateAction != Paste && candidateAction != CreateNode) {
			...
			} else if (control.HotObject && control.HotObject->IsSelectable() && !
				IsGroup(control.HotObject->AsNode())) {
			...
			const auto hasAnyLinks = std::any_of(m_Context.begin(), m_Context.end(),
			                                     [](Object *object) { return object->AsLink() != nullptr; });
			if (!hasAnyLinks && m_Context.size() > 1) // one node cannot make connection to anything
			{
				...
				std::for_each(m_Context.begin(), m_Context.end(), [&nodes](Object *object) {
					if (auto node = object->AsNode()) nodes.push_back({node->m_ID, node});
				});
				...
				std::erase_if(links, [&isNodeInContext](Link *link) {
					return !isNodeInContext(link->m_StartPin->m_Node->m_ID) || !isNodeInContext(
						link->m_EndPin->m_Node->m_ID);
				});
...
bool ed::CreateItemAction::Process(const Control &control) {
	...
	if (m_DraggedPin && control.ActivePin == m_DraggedPin && (m_CurrentStage == Possible)) {
		...
		Pin cursorPin(Editor, 0, draggingFromSource ? PinKind::Input : PinKind::Output);
		...
		Pin *&freePin = draggingFromSource ? candidate.m_EndPin : candidate.m_StartPin;
...
ed::EditorAction::AcceptResult ed::DeleteItemsAction::Accept(const Control &control) {
	...
	auto &io = ImGui::GetIO();
	if (Editor->CanAcceptUserInput() && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Delete)) && Editor->
		AreShortcutsEnabled()) {
		auto &selection = Editor->GetSelectedObjects();
		if (!selection.empty()) {
...
bool ed::DeleteItemsAction::QueryItem(ObjectId *itemId, IteratorType itemType) {
	...
	auto itemCount = static_cast<int>(m_CandidateObjects.size());
...
const char* ed::Style::GetColorName(StyleColor colorIndex) const {
	switch (colorIndex) {
		...
		case StyleColor_HighlightLinkBorder:
...
float* ed::Style::GetVarFloatAddr(StyleVar idx) {
	switch (idx) {
		...
		case StyleVar_HighlightConnectedLinks:
			return &HighlightConnectedLinks;
		case StyleVar_SnapLinkToPinDir:
			return &SnapLinkToPinDir;
		case StyleVar_HoveredNodeBorderOffset:
			return &HoverNodeBorderOffset;
		case StyleVar_SelectedNodeBorderOffset:
			return &SelectedNodeBorderOffset;
		default:
			return nullptr;
	}
}
...
std::string ed::Config::Load() {
	...
	if (LoadSettings) {
		...
		if (size > 0) {
			data.resize(size);
			LoadSettings(data.data(), UserPointer);
		}
...
std::string ed::Config::LoadNode(NodeId nodeId) {
	...
	if (LoadNodeSettings) {
		...
		if (size > 0) {
			data.resize(size);
			LoadNodeSettings(nodeId, data.data(), UserPointer);
		}
	}
