...
void Windows::CheckpointsWindow::createNewElementFrom(int i) {
	const auto& checkpoint = ProjectsService::getInstance().getCurrentProject().getCurrentScene()->getCheckpoints().at(i);
	ProjectsService::getInstance().getCurrentProject().getCurrentScene()->createNewCheckpointFrom(*checkpoint);
	ProjectsService::getInstance().getCurrentProject().getCurrentScene()->getCheckpoints().back()->spawnEditorCheckpoint();
}
...
void Windows::CheckpointsWindow::deleteElement(int i) {
	ProjectsService::getInstance().getCurrentProject().getCurrentScene()->getCheckpoints().at(i)->deleteEditorCheckpoint();
...
void Windows::CheckpointsWindow::drawOptions() {
	...
	Checkpoint* checkpoint = ProjectsService::getInstance().getCurrentProject().getCurrentScene()->getCheckpoints().at(this->currentElement).get();
	...
	if (ImGui::SliderInt(local.get("general.type").c_str(), &checkpoint->getType(), 0, 1, local.getArray("checkpoint.types")[checkpoint->getType()].c_str())) {
	...
	if (checkpoint->getType() == 0) {
		changed |= ImGui::SliderInt(local.get("checkpoint.type_sphere").c_str(), &checkpoint->getCheckpointType(), 0, 6);

		changed |= ImGui::ColorEdit4(local.get("checkpoint.color").c_str(), checkpoint->getColor().data());
		changed |= ImGui::DragInt(local.get("checkpoint_objective.pulse_period").c_str(), &checkpoint->getPulsePeriod(), 64, 0, USHRT_MAX);
		changed |= ImGui::InputFloat(local.get("checkpoint_objective.pulse_fraction").c_str(), &checkpoint->getPulseFraction(), .0f, .0f);
		changed |= ImGui::DragInt(local.get("checkpoint_objective.rotate_rate").c_str(), &checkpoint->getRotateRate(), 10, SHRT_MIN, SHRT_MAX);
		...
		if (ImGui::SliderInt(local.get("checkpoint.type_sphere").c_str(), &checkpoint->getCheckpointType(), 0, 8))
			checkpoint->spawnEditorCheckpoint();
		changed |= ImGui::DragFloat(local.get("general.rotate").c_str(), &checkpoint->getAngle(), 0.1f, -180.f, 180.f, "%.2f°");
	...
	if (ImGui::SliderInt(local.get("general.type_marker").c_str(), &checkpoint->getBlipType(), 0, 1, local.getArray("general.type_marker_enum").at(checkpoint->getBlipType()).c_str()))
		checkpoint->spawnEditorBlip();

	if (checkpoint->getBlipType() == 0) {
		if (utils::Combo(local.get("general.color_marker").c_str(), &checkpoint->getBlipColor(), local.getArray("general.color_marker_enum"), 9)) {
			checkpoint->spawnEditorBlip();
		}
	} else {
