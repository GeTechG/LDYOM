...
void Windows::ConsoleWindow::draw() {
	...
	if (ImGui::Begin(fmt::format("{} {}", ICON_FA_TERMINAL, local.get("console_window.title")).c_str())) {
		...
		ImGui::InputTextMultiline("##codeInput", code, std::size(code),
		                          ImVec2(-ImGui::GetFontSize() * 5.f + 4.f, 100.f));
		ImGui::SameLine();
		if (ImGui::Button(local.get("console_window.run").c_str(), ImVec2(ImGui::GetFontSize() * 5.f, 0.f))) {
			if (std::strcmp(code, "clear") == 0) {
				...
				LuaEngine::getInstance().getLuaState().safe_script(
					code, [](lua_State *, sol::protected_function_result pfr) {
