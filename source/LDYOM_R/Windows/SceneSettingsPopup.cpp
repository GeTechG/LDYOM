...
void Windows::SceneSettingsWindowPopup::draw() {
	...
	if (ImGui::BeginPopupModal(fmt::format("{} {}", ICON_FA_SLIDERS_H, local.get("scene_settings.title")).c_str(), &this->show_, ImGuiWindowFlags_AlwaysAutoResize)) {
		...
		ImGui::BeginDisabled(!ProjectsService::getInstance().getCurrentProject().getCurrentScene()->isToggleSceneSettings());
			...
			if (ImGui::TreeNode(fmt::format("{} {}", local.get("actor.group"), i + 1).c_str())) {
				for (int j = 0; j < 9; j++) {
					if (sceneSettings.groupRelations[i][j] != -1) {
						...
						ImGui::SliderInt(groupName.c_str(), &sceneSettings.groupRelations[i][j], 0, 4, local.getArray("scene_settings.relationship_types").at(sceneSettings.groupRelations[i][j]).c_str());
					}
				}
				ImGui::TreePop();
			}
		...
		timeChanged |= ImGui::SliderScalar(local.get("general.hours").c_str(), ImGuiDataType_S8, &sceneSettings.time[0], &hourMin, &hourMax);
		ImGui::SameLine();
		constexpr auto minutesMin = static_cast<ImS8>(0);
		constexpr auto minutesMax = static_cast<ImS8>(59);
		ImGui::SetNextItemWidth(ImGui::GetFontSize() * 5.f);
		timeChanged |= ImGui::SliderScalar(local.get("general.minutes").c_str(), ImGuiDataType_S8, &sceneSettings.time[1], &minutesMin, &minutesMax);
		...
		ImGui::DragFloat(fmt::format("{} {}", ICON_FA_TRAFFIC_CONE, local.get("general.ped_traffic")).c_str(), &sceneSettings.trafficPed, 0.01f, 0.f, 2.f);
		ImGui::DragFloat(fmt::format("{} {}", ICON_FA_TRAFFIC_CONE, local.get("general.car_traffic")).c_str(), &sceneSettings.trafficCar, 0.01f, 0.f, 2.f);

		ImGui::Dummy(ImVec2(0.f, ImGui::GetFontSize()));
		
		ImGui::SliderInt(fmt::format("{} {}", ICON_FA_STAR, local.get("general.min_wanted")).c_str(), &sceneSettings.wantedMin, 0, 6);
		ImGui::SliderInt(fmt::format("{} {}", ICON_FA_STAR, local.get("general.max_wanted")).c_str(), &sceneSettings.wantedMax, 0, 6);

		ImGui::Dummy(ImVec2(0.f, ImGui::GetFontSize()));

		if (ImGui::DragInt(fmt::format("{} {}", ICON_FA_SUN_CLOUD, local.get("general.weather")).c_str(), &sceneSettings.weather, 0.1f, 0, 255)) {
			CWeather::ForceWeatherNow(static_cast<short>(sceneSettings.weather));
		}

		ImGui::Dummy(ImVec2(0.f, ImGui::GetFontSize()));

		utils::ToggleButton(fmt::format("{} {}", ICON_FA_SKULL_CROSSBONES, local.get("general.riot_mode")).c_str(), &sceneSettings.riot);
